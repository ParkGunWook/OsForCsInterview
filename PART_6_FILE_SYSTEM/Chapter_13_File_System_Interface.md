...
두 프로그래머가 겹치는 프로젝트를 진행중이라고 생각하겠다. 프로젝트와 관련된 파일들은 다른 프로젝트와 두 프로그래머의 파일로부터 분리된 서브디렉토리에 저장된다. 그러나 두 프로그래머들이 프로젝트에 똑같이 관여한다면, 두 프로그래머는 그들의 개인 디렉토리에 서브디렉토리가 있기를 원한다. 이런 상황에서, 일반적인 서브디렉토리는 공유된다. 공유되는 디렉토리 또는 파일은 한번에 두 장소의 파일 시스템에 존재한다.

트리 구조는 파일 또는 디렉토리의 공유를 방지한다. **Accyclic graph**는 사이클이 없는 그래프이고 디렉토리가 서브디렉토리와 파일을 공유하게해준다. 같은 파일 또는 서브디렉토리는 다른 디렉토리에 존재할 것이다. 비순환 그래프는 트리 구조 디렉토리의 자연적인 일반화이다.

공유 파일은 파일의 두 카피와는 같지 않다. 두개의 카피에서, 각 프로그래머는 원본보다는 카피본을 보지만, 만약 프로그래머가 파일을 바꾸면, 변화는 다른이의 카피에 나타나지 않는다. 공유 파일에서는, 오직 한개의 실제 파일만이 존재하고, 그래서 한 사람이 만든 파일의 변화는 즉시 다른이에게 보인다. 공유는 특히 서브디렉토리에 중요하다. 한 사람에 의해서 생긴 새로운 파일은 공유되는 모든 서브디렉토리에 나타날 것이다. 

사람들이 팀으로 일할때, 그들이 원하는 모든 파일들은 한개의 디렉토리에 두고 공유되기를 원한다. 각 팀 멤버의 홈 디렉토리는 서브디렉토리로서 공유된 파일의 디렉토리를 포함한다. 단일 유저의 경우에도, 유저의 파일 구성은 다른 서브 디렉토리를 필요로 할 수 있다. 예를 들어서, 특정 파일을 위해서 쓰인 프로그램은 반드시 모든 프로그램의 디렉토리와 프로젝트의 디렉토리에 있어야한다.

공유 파일과 서브디렉토리는 다양한 방식으로 구현된다. 일반적인 방법은, 유닉스 시스템을 예로 들겠다. 유닉스는 링크라고 불리는 새로운 디렉토리 엔트리르 만든다. **link**는 다른 파일 또는 서브디렉토리를 가르키는 포인터이다. 예를 들어서, 링크는 절대 또는 상대 경로 이름으로 구현된다. 파일로의 참조가 만들어지면, 우리는 디렉토리를 탐색한다. 만약 디렉토리 엔트리가 링크로 마크되면, 실제 파일의 이름에 링크 정보가 포함된다. 우리는 링크를 패스 이름을 이용해서 실제 파일을 위치하기 위해서 **resolve**한다. 링크들은 디렉토리 엔트리의 포맷에 의해서 쉽게 구별되고 효과적으로 비간접적인 포인터이다. 운영 체제는 디렉토리 링크를 순회할때 시스템의 비순환 구조를 보존하기 위해서 이런 링크들을 무시한다.

다른 일반적인 파일 공유를 구현하는 방법은 단순히 공유하는 두개의 디렉토리에 대한 모든 정보를 복사하는 것이다. 그러므로, 두 엔트리는 동일하다. 이 방법과 링크의 생성의 차이는 다음과 같다. 디렉토리 엔트리를 복사하면, 원본과 복사본은 구별이 불가능하다. 디렉토리 엔트리를 복제하는 주요한 문제는 파일이 수정되었을 때 일관성을 유지하는 것이다.

비순환 디렉토리 구조는 단순한 트리 구조보다는 더 유연하다. 그러나 더욱 복잡하다. 몇가지 문제가 조심히 다루어져야한다. 파일이 이제 다양한 절대경로를 가진다. 결과적으로, 다른 파일이 같은 파일일 수 있다. 이런 상황은 프로그래밍언어 문제를 얼라이어싱하는 것과 같다. 만약 우리가 파일을 찾고 모든 파일의 통계를 축적하거나 모든 파일을 백업 저장소에 카피할 때 전체 파일 시스템을 순회하려고 하면, 이 문제는 우리가 공유된 구조를 한번이상 순회하지 않기를 원하기에 중대해진다.

다른 문제는 삭제를 포함한다. 공유 파일에 할당된 공간은 언제 해제되고 재사용이 될까? 한가지 가능성은 누군가가 지웠을때 파일을 삭제하는 것이고, 그러나 이 활동은 현재 존재하지 않는 파일을 떠다니는 포인터로 남겨둘 수 있다. 더 나쁘면, 만약 남은 파일 포인터가 실제 디스크 주소를 포함하면, 공간은 다른 파일에 의해서 사용되고, 이런 떠다니는 포인터는 다른 파일의 중간에 위치할 것이다.

심벌릭 링크로 공유가 구현된 시스템에서, 이런 상황은 쉽게 다룰 수 있다. 링크의 삭제는 실제 파일에는 영향을 끼치지 않는다. 오직 링크만 제거된다. 만약 파일 엔트리가 삭제되면, 파일에 할당된 공간은 해제되고 링크를 떠다니게 둔다. 우리는 이런 링크를 검색하고 삭제한다. 그러나 관련된 링크의 리스트가 각 파일에 보관되기에, 이 검색은 값이 비싸다. 대안으로, 우리는 그들에 대한 시도가 생기기전까지는 링크를 내버려둔다. 그떄에, 우리는 링크에 의해서 주어진 파일의 이름이 존재하지 않고 링크 이름을 resolve하는데 실패했다고 결정한다. 접근은 불가능한 파일 접근으로 처리된다.(이런 경우에, 시스템 디자이너는 파일이 삭제되었을때 무엇을 하고 워리지널 파일의 심벌릭 링크가 사용되기 전에 같은 이름의 다른 파일이 생기는 것을 고려한다.) 유닉스의 경우에는, 심벌릭 링크든 파일이 삭제될때 내버려두고, 원본 파일이 사라지고 교체된 것을 깨닫는 것은 유저에게 달려있다. MS 또한 이 같은 방법을 사용한다.

삭제에 대한 다른 접근은 모든 레퍼런스가 삭제될떄까지 파일을 유지하는 것이다. 이 접근을 구현하려면, 우리는 반드시 마지막 참조가 삭제된 것을 결정하는 메커니즘을 가져야한다. 우리는 라일에 모든 레퍼런스의 리스트를 유지해야한다. 링크 또는 디렉토리의 카피가 생기면, 새로운 엔트리가 파일 레퍼런스 리스트에 추가된다. 링크 또는 디렉토리 엔트리가 삭제되면, 우리는 리스트에서 그것의 엔트리를 지운다. 파일은 파일 레퍼런스 리스트가 비워지면 삭제한다.

이 접근의 문제는 파일 레퍼런스의 리스트가 너무 커지는 경우이다. 그러나, 우리는 전체 리스트를 유지할 필요는 없고 우리는 오직 레퍼런스의 수만 세면된다. 새로운 링크 또는 디렉토리 엔트리는 참조수를 증가시킨다. 링크 또는 엔트리 삭제는 카운트를 줄인다. 카운트가 0이되면 파일은 삭제된다. 유닉스 운영체제는 넌심볼릭 링크(**hard link**)를 이 접근에 사용했고 파일 정보 블럭(inode)에 참조수를 저장했다. 디렉토리에 다중 참조를 효과적으로 방지하면서, 우리는 비순환 그래프 구조를 유지했다.

이런 문제를 피하기위해서, 몇몇 운영체제는 공유 디렉토리 또는 링크를 허용하지 않는다.

### 13.3.5 General Graph Directory

비순환 그래프 구조를 사용하는 심각한 문제는 사이클이 없다고 확인하는 것이다. 만약 우리가 2단 디렉토리에서 시작해서 유자거 서브디렉토리를 만들게하면, 트리 구조 디렉토리이다. 단순히 새로운 파일과 서브디렉토리를 존재하는 트리 구조에 추가하는 것은 트리 구조를 유지하게 해준다. 그러나, 우리가 링크를 추가하면, 트리 구조는 파괴되고, 단순한 그래프 구조가 된다.

비순환 그래프의 장점은 그래프를 탐색하는 것즘과 더이상 파일에 레퍼런스가 없다는 것을 결정하는 알고리즘이 간단하다는 것이다. 우리는 비순환 그래프에서 성능을 이유로 공유된 부분을 두번 방문하는 것을 회피한다. 만약 우리가 특정 파일을 찾지못하고 주요한 공유 디렉토리를 검색하면, 우리는 서브디렉토리를 다시 찾는 것을 피하기를 원한다. 두번쨰 검색은 시간 낭비이다.

만약 디렉토리에서 사이클이 존재하면, 우리는 어떠한 컴포넌트도 두번 검색하는 것을 피하는데, 정확성 뿐만이 아니라 성능 때문이다. 나쁘게 디자인된 알고리즘은 사이클을 무한히 탐색하면서 끝나지 않는다. 검색동안 접근한 디렉토리의 수를 제한하는 것은 한가지 해결책이다.

비슷한 문제는 언제 파일을 삭제될 수 있는지 결정할 떄도 존재한다. 비순환 그래프 디렉토리 구조에서, 참조수 0은 파일 또는 디렉토리에 더이상의 참조는 없고 파일은 삭제된다. 그러나, 사이클이 존재하면, 레퍼런스 카운트는 더이상 디렉토리 또는 파일을 참고할수 없어도 0이 되지 않는다. 이런 변칙은 디렉토리 구조의 자기 자신 가르키기에 의해서 생길 수 있다. 이런 경우에, 우리는 **garbage collection**을 사용해서 언제 마지막 레퍼런스가 삭제되었고 디스크 공간이 재할당되었는지 판단한다. 가비지 컬렉션은 전체 파일 시스템을 순회하는 것을 포함하고, 접근 가능한 모든것을 마크하게 해준다. 그때, 두번째 기능은 여유 공간의 리스트에서 마크되지 않은 모든 것을 수집한다.(이런 마킹 과정은 파일 시스템의 모든 것들이 한번만 탐색되고 검색되게 해준다.) 디스크 베이스 파일 시스템에서의 가비지 컬렉션은 매우 시간을 많이 잡아먹기에 가끔 시도된다.

GC는 오직 그래프에 사이클이 있을 가능성이 있을 떄 필요하다. 그러므로, 비순환 그래프 구조는 훨씬 간단하다. 어려운점은 새로운 링크가 사이클을 만들지 않는 것이다. 우리는 어떻게 새로운 링크가 사이클을 만드는지 알수 있을까? 순환을 찾는 알고리즘이 존재하지만 계산은 비싸고, 특히 그래프가 디스크 공간에 있다면 더 비싸다. 단순한 알고리즘은 디렉토리와 링크의 특정한 경우에 디렉토리 순회중에 링크를 우회하는 것이다. 사이클은 피해지고, 추가적인 오버헤드는 필요하지 않다.

## 13.4 보호

정보가 컴퓨터 시스템에 저장되면, 우리는 물리적 충격과 적절하지 않은 접근으로부터 안전하게 보관해야한다.

신뢰성은 파일의 카피를 복제함으로서 제공한다. 많은 컴퓨터는 카피를 유지하는 적절한 간격으로 디스크 파일을 자동으로 디스크 파일에서 테이프로 복사하는 시스템 프로그램을 가진다. 파일 시스템이 사고로 삭제될때를 대비한다. 파일 시스템이 하드웨어 문제, 파워 서지 실패, 헤드 크래시, 온도 문제로 손상될 수 있다. 파일은 사고록 삭제된다. 파일 시스템 소프트웨어의 버그는 파일 컨텐츠를 잃게한다. 신뢰성은 11장에서 더 다루었다.

보호는 다양한 방식으로 제공된다. 현대 운영체제를 운영하는 랩탑에서, 우리는 접근하기 위한 유저 이름과 암호 인증, 랩탑을 열고 드라이브를 삭제하는 누군가가 그것의 데이터에 접근하는데 시간을 걸리게하는 2차 저장소 암호화와 네트워크를 통해서 침입하려는데 힘들게하는 방화벽을 이용해서 보호를 제공한다. 멀티 유저 시스템에서, 시스템의 적절한 접근또한 데이터의 접근을 허용하기 위해서 진보된 메커니즘을 필요로한다.

### 13.4.1 접근의 타입

파일을 보호하는 필요는 파일에 접근하는 능력의 직접적인 결과이다. 다른 유저의 접근을 허용하지 않는 시스템은 보호가 필요하지 않다. 그러므로, 우리는 접근을 방지함으로서 완성된 보호를 제공한다. 대안으로, 우리는 보호 없이 접근을 제공할 수 있다. 두 접근은 매우 일반적으로 사용된다. 우리가 필요한 것은 제어된 접근이다.

보호 메커니즘은 만들 수 있는 파일 접근의 타입을 제한함으로서 제공된다. 접근은 몇자기 요소로 허용되거나 거부된다. 다양한 명령어가 제어된다.
- Read : 파일을 읽는다.
- Write : 파일을 쓴다.
- Execute : 메모리로 파일을 로드하고 실행한다.
- Append : 파일의 끝에 새로운 정보를 쓴다.
- Delete : 파일을 삭제하고 재사용을 위해서 공간을 해제한다
- List : 파일의 이름과 성질을 리스트한다.
- Attribute change : 파일의 성질을 바꾼다.

renaming, copying, editing 같은 명령어 또한 제어할 수 있다. 많은 시스템에서, 이런 고레벨 함수들은 저레벨 시스템 콜을 만드는 시스템 프로그램에 의해서 구현된다. 보호는 오직 저레벨에서만 제공된다. 예를 들어서, 파일을 복사하는 것은 단순히 읽기 요청의 시퀀스로 구현될 수 있다. 이런 경우에, 읽기 접근을 한 유저는 파일을 카피하고 프린트할 수 있다.

많은 보호 메커니즘이 제안되었다. 각각은 장점과 단점이 있고 반드시 의도한 앱에 적절해야한다. 소수의 리서치 그룹 멤버가 사용하는 작은 컴퓨터 시스템을 예로 들면, 그들은 연구, 금융에 사용되는 큰 기업 컴퓨터 만큼의 보호를 필요로 하지 않는다. 우리는 이런 접근을 17장에서 마무리 짓겠다.

### 13.4.2 접근 컨트롤

보호 문제에 일반적인 접근은 유저의 신원에 따라서 접근을 만드는 것이다. 다른 유저는 파일 또는 디렉토리에 다른 타입의 접근이 필요할 것이다. 가장 일반적인 신원에 따른 접근을 구현하는 구조는 각 파일과 디렉토리를 유저 이름과 각 유저에게 허용된 접근의 타입을 명세하는 **access control list(ACL)**과 연관해서 구현하는 것이다. 유저가 특정 파일에 접근을 요청하면, 운영체제는 파일과 연관된 접근 리스트를 체크한다. 만약 유저가 요청한 접근에 리스트되어있으면, 접근이 허용된다. 다른 경우에는, 접근 위반이 생기고, 유저 잡은 파일에 접근이 거부된다.

이런 접근은 복잡한 접근 방법을 가능하게하는 장점이 있다. 접근 리스트의 주된 문제는 그들의 길이이다. 만약 우리가 모든 사람이 파일을 읽게하려면, 우리는 반드시 모든 유저를 읽기 접근과함께 리스트해야한다. 이런 테크닉은 두가지 원치않는 결과를 가진다.
- 이런 리스트를 건설하는 것은 지루하고 보상없는 태스크인데, 특히 만약 우리가 시스템의 유저의 리스트를 모를 때이다.
- 디렉토리 엔트리는 고정된 사이즈인데, 이제 반드시 사이즈에 가변이어야하고, 더욱 복잡한 공간 관리가 필요해진다.

이런 문제들은 엑세스 리스트의 엑세스 리스트의 합쳐진 버전의 사용으로 해결된다.

엑세스 컨트롤 리스트의 길이를 압축하기 위해서, 많은 시스템들이 유저의 3가지 분류를 했다.

- Owner : 파일을 만든 유저가 주인이다.
- Group : 파일을 공유하고 비슷한 엑세스를 필요로하는 유저의 집합이 그룹 또는 워크 그룹이다.
- Other : 시스템의 다른 유저이다.

가장 일반적인 현대의 접근은 엑세스 컨트롤 리스트를 가장 일반적인 3가지 구조로 표현하는 것이다. 예를 들어서, 솔라리스는 3가지 카테고리의 엑세스를 사용하지만 정제된 엑세스 컨트롤이 필요할 때 특정 파일과 디렉토리에 엑세스 컨트롤 리스트를 붙이는 걸 허용한다.

새로운 책을 쓰는 사라를 예시로 들겠다. 그녀는 프로젝트를 위해서 3명의 졸업생을 고용했다. 책의 텍스트는 book.tex에 저장된다. 이 파일에 대한 보호는 다음과 같다.

- 사라는 반드시 파일에 대한 모든 접근을 실행할 수 있어야한다.
- 피고용인들은 파일을 읽고 쓸수 있어야한다. 그들은 삭제는 못한다.
- 모든 유저들은 읽을 수 있지만, 쓸 수 잆다.

이런 보호를 수행하기 위해서, 우리는 피고용인을 위한 새로운 그룹을 만들어야한다.그룹의 이름은 반드시 파일 book.tex와 관련있어야하고 접근 권한은 우리가 말한 정책에 부합해야한다.

이제 사라는 방문자들에게 1장에 대한 임시 접근을 허용해야한다. 방문자들은 모든 챕터에 접근을 주는 피고용인 그룹에 추가되지 않는다. 파일은 오직 한 그룹에 존재할 수 있고, 사라는 다른 그룹에게 1장을 추가하지 않는다. 접근 컨트롤 리스트 기능에서, 방문자들은 1장의 엑세스 컨트롤 리스트에 추가된다.

적절히 작동하는 이런 구조에서, 허용과 접근 리스트는 단단히 컨트롤 되어야한다. 이 컨트롤은 여러가지 방법으로 구현된다. 유닉스 시스템에서, 그룹들은 기관의 매니저에 의해서 생성되고 수정된다. 그러므로, 컨트롤은 사람에 의해서 수행된다. 엑세스 리스트는 17.6.2절에서 더욱 다루겠다.

더욱 제한된 보호 분류와 함께, 오직 3가지 필드가 보호를 정의할 필요가 있다. 자주, 각 필드는 비트의 집합이고, 각 비트는 접근과 관련된 것을 허용하거나 방지한다. 예를 들어 유닉스 시스템은 각각을 3가지 비트 필트 `rwx`로 구현한다. 분리된 필드는 파일 오너, 파일의 그룹, 그외로 구분된다. 이 구조에서 파일마다 9개의 비트가 보호 정보를 저장하기 위해서 필요하다. 그러므로, 에를 들어서, book.tex를 위한 보호 필드는 사라에게는 rwx가 허용되고, 피고용인은 rw가 허용되고 그외는 r만 허용된다.

이런 접근을 합치는 한가지 어려움은 유저 인터페이스와 관련된다. 유저들은 부가적인 ACL 허용이 파일에 설정될때 알아야한다. 솔라시스를 예시로 들면 "+"가 일반적인 허용에 다음과 같이 추가된다.

`19 -rw-r--r--+ 1 jim staff 130 May 25 22:13 file1`

`setfac1`과 `getfac1`이 ACL을 관리하기 위해서 사용된다.

윈도우 유저는 접근 컨트롤을 간단히 GUI를 통해서 한다. 

### 13.4.3 다른 보호 접근

보호에 대한 다른 접근은 각 파일에 암호를 사용하는 것이다. 컴퓨터 시스템으로의 접근이 암호에 의해서 제어되는 것처럼, 각 파일의 접근은 같은 방식으로 제어된다. 만약 암호가 랜덤으로 선택되고 자주 바뀌면, 이 구조는 파일에 대한 접근을 제한하는데 효과적이다. 암호의 사용은 다음과 같은 단점이 있다. 유저가 사용해야하는 암호의 수가 커지면, 구조가 비효율적이게된다. 두번째, 오직 하나의 암호가 모든 파일에 사용되고, 한번 찾으면, 모든 파일이 뚫린다. 보호는 하나 또는 없는 것이다. 몇몇 시스템은 이런 문제를 해결하기 위해서 개인 파일보다는 서브디렉토리에 암호를 허용한다. 파티션 또는 개인 파일의 암호화는 강력한 보호를 제공하지만, 암호 관리가 문제이다.

멀티레벨 디렉토리 구조에서, 우리는 개인 파일뿐만이 아니라 서브디렉토리의 파일의 집합도 보호해야한다. 즉, 우리는 디렉토리 보호를 위한 메커니즘을 제공해야한다. 보호를 위한 파일 명령어는 디렉토리 보호 명령어와 조금 다르다. 우리는 디렉토리의 파일의 생성과 삭제를 컨트롤한다. 추가적으로, 우리는 유저가 디렉토리의 파일의 존재를 결정하는 컨트롤을 원한다. 가끔, 존재의 지식과 파일의 이름은 그 자체로 중대하다. 그러므로, 디렉토리의 컨텐츠 리스팅은 반드시 보호된 명령어야한다. 비슷하게, 만약 패스 이름이 디렉토리의 파일을 언급하면, 유저는 반드시 디렉토리와 파일에 대한 접근을 허용받아야한다. 파일이 다양한 패스이름을 가진 시스템에서, 패스이름에 따라서 유저는 특정 파일에 다른 엑세스 권한을 가진다.

## 13.5 Memory-Mapped Files

파일에 접근하는 다른 메서드가 있는데, 그것은 매우 흔히 사용된다. 표준 시스템 콜 `open(), read(), write()`를 사용하는 디스크에서 파일의 순차적 읽기를 고려하겠다. 각 파일 접근은 시스템 콜과 디스크 접근을 필요로한다. 대안으로, 우리는 10장에서 말한 I/O를 루틴 메모리 엑세스처럼 다루는 가상 메모리를 사용하겠다. 이 접근은 **memory mapping**이라고 하고, 가상 주소 공간의 일부분이 파일과 논리적으로 연관되어 있다. 우리가 보았듯이, 이것은 엄청난 성능 향샹을 이끌 수 있다.

### 13.5.1 기본 메커니즘

메모리 매핑 파일은 디스크 블록을 메모리의 페이지에 매핑하는 것이다. 파일에 첫번째 접근은 일반적인 디맨드 페이징을 통해서 진행된다. 그러나, 파일의 페이지 사이즈 부분은 파일 시스템에서 물리 페이지로 읽힌다.(몇몇 시스템들은 한번에 메모리의 페이지 사이즈 청크보다 크게 읽기를 선택할 수 있다.) 파일에 연속적 읽기와 쓰기는 루틴 메모리 접근으로 관리된다. `read()`와 `write()` 시스템콜을 이용하는 오버헤드를 일으키는 것보다 메모리를 통해 파일을 관리하는 것은 파일 접근과 사용을 단순화하고 가속시킨다.

메모리에 매핑된 파일을 2차 저장소에 쓰는 것은 즉각적인 쓰기를 필요로하지 않는다. 일반적으로, 시스템들은 오직 파일이 닫힐때만 파일을 메모리 이미지로 업데이트한다. 메모리 압박때문에, 운영체제들은 다른 사용때문에 메모리를 해제할때 스왑 스페이스로 당장 변화시킨다. 파일이 닫히면, 모든 메모리 맵드 데이터들은 2차저장소의 파일에 다시 쓰이고 프로세스의 가상메모리에서 지워진다.

몇몇 운영체제들은 메모리 매핑을 오직 특정한 시스템 콜로 제공하고 다른 모든 파일 I/O를 수행하기 위해서 표준 시스템 콜을 사용한다. 그러나, 몇몇 운영체제들은 파일을 어떤 파일이 메모리 맵드로 명세되었는지에 관계없이 메모리 매핑하도록 선택한다. 솔라리스를 예시로 들겠다. 만약 파일이 메모리 맵드로 명시되면, 솔라리스는 파일을 프로세스의 주소공간으로 매핑한다. 만약 시스템콜을 이용해서 파일이 열리고 접근되면 솔라시스는 여전히 파일을 메모리 맵한다. 그러나, 파일은 커널 주소 공간으로 매핑된다. 어떻게 파일이 열렸든지, 솔라리스는 모든 파일 I/O를 메모리 맵으로 취급하고, 효율적인 메모리 서브시스템을 통해서 파일 접근을 허용하고 각 전통적인 `read()`와 `write()`에의한 시스템콜 오버헤드를 피한다.

다중 프로세스들은 같은 파일을 동시에 매핑하게 허용되고, 데이터의 공유를 허용한다. 어떤 프로세스의 쓰기는 가상 메모리의 데이터를 수정하고 같은 파일의 섹션을 매핑하는 다른 프로세스에게 보여진다. 예전에 본 가상 메모리에서, 어떻게 메모리의 메모리 맵드 섹션의 메모리의 공유가 구현될지는 명확해야한다. 각 공유 프로세스의 가상 메모리 맵은 같은 물리 메모리의 페이지를 가르킨다. 디스크 블럭의 카피를 잡고있는 페이지이다. 메모리 매핑 시스템 콜들은 COW 함수로 지원되고, 프로세스가 읽기 전용 모드만 파일을 공유하게 허용해서 그들이 수정하는 데이터의 카피를 가지게한다. 그래서 공유된 데이터의 접근이 조정되고, 포함된 프로세스들은 6장에서 말한 상호배제를 위한 메커니즘을 사용할 것이다.

꽤 자주, 공유 메모리는 실제로 메모리 매핑 파일에 구현된다. 이 시나리오에서, 프로세스들은 통신하는 프로세스를 그들의 가상 주소 공간에 메모리 매핑함으로서 공유 메모리를 사용해서 통신할 수 있다. 메모리 맵드 파일은 통신하는 프로세스간에 공유된 메모리의 리전을 가져온다. 우리는 이미 이것을 3.5절에서 보았고, POSIX 공유 메모리 객체가 생성되고 각 통신 프로세스 메모리는 그것의 주소공간에 객체를 매핑한다. 다음 절에서, 우리는 메모리 맵드 파일을 이용해서 공유 메모리를 지원하는 윈도우즈 API를 보겠다.

### 13.5.2 윈도우즈 API에서의 공유 메모리

윈도우 API에서 메모리 맵드 파일을 이용해서 공유 메모리의 리전을 만드는 일반적인 개요는 먼저 파일의 **file mapping**을 만들고 프로세스의 가상 주소 공간에 이는 맵드 파일의 **view**를 만드는 것이다. 두번째 프로세스는 그것의 가상 주소 공간에서 맵드 파일의 뷰를 열고 생성할 수 있다. 맵드 파일은 프로세스 간에 통신을 가능하게하는 공유 메모리 객체를 대표한다.

우리는 다음으로 이런 과정을 더욱 자세히 살펴보겠다. 이 예시에서, 생산자 프로세스는 먼저 메모리 매핑 기능을 이용해서 공유 메모리 객체를 만든다. 생산자는 다음에 공유 메모리에 메시지를 쓴다. 그 후에, 소비자 프로세스는 공유 메모리 객체에 매핑을 열고 메시지를 읽는다.

메모리 맵드 파일을 만들기 위해서, 프로세스는 먼저 매핑될 파일을 `CreateFile()` 함수를 이용해서 열고, 열린 파일의 `HANDLE`을 리턴받는다. 프로세스는 그후에 `CreateFileMapping()`을 이용해서 이 파일의 매핑을 만든다. 맵드 파일의 뷰는 프로세스의 가상 주소 공간에 매핑된 파일의 일부분을 대표한다. 전체 파일 또는 그것의 일부분만이 매핑된다.

`CreateFileMapping()`은 SharedObject라고 불리는 **named shared-memor object**를 만든다. 소비자 프로세스는 같은 이름의 오브젝트로의 매핑을 만들어서 이 공유 메모리 세그먼트를 이용해서 통신한다. 생산자는 그것의 가상 주소 공간에서 메모리 맵드 파일의 뷰를 만든다. 3개의 파라미터에 0을 줌으로서, 그것은 전체 파일이 맵드 뷰라고 한다. 그것은 대신에 특정한 오프셋이나 사이즈로 주어질 수 있다.(전체 매핑이 메모리에 로드되지 않는 것은 중요하다. 오히려, 맵드 파일은 디맨드 페이지이고, 그러므로 오직 그들이 접근될떄만 메모리에 페이지를 가져와야한다.) `MapViewOfFile()`함수는 공유 메모리 객체의 포인터로 리턴한다. 이 메모리 공간으로의 어떠한 접근도 메모리 맵드 파일이다. 예를 들어, 생산자 프로세스는 "Shared memory message"를 공유 메모리에 쓴다.

소비자 프로그램은 생산자보다 보통 간단하다. 필요한 것은 오직 존재하는 공유 메모리 객체를 생성하는 것이다. 소비자 프로세스는 또한 맵드 파일의 뷰를 생성한다. 컨슈머는 그 이후에 공유 메모리에 쓰인 것을 읽는다.

마지막으로 프로세스는 맵드 파일의 뷰를 제거한다.