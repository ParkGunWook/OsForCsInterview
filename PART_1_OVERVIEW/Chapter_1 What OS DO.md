...
모바일 컴퓨팅은 스마트폰이나 태블릿 컴퓨터의 컴퓨팅을 의미한다. 이런 기기들은 가볍고 들고다니기 쉬운 형태로 구분이 된다. 역사적으로 데스크탑과 랩톱 컴퓨터를 비교하면 모바일 시스템은 스크린 사이즈, 메모리 용량, 전체적인 기능을 한손에 들수 있는 모바일 기기로 메일이나 웹브라우징을 지원했다. 최근 몇넌 동안, 모바일 기기는 여러가지 기능을 지원하게 되었고, 소비자 랩탑과 태블릿 컴퓨터는 구별하기 어려워졌다. 사실 우리는 현대의 모바일 기기는 다양한 기능을 많이 지원했다. 

오늘날, 모바일 시스템은 이메일이나 웹브라우징만이 아니라 음악, 비디오 재생, 디지털 책 읽기, 사진찍기, 고품질 영상 저장과 편집을 수행한다. 그에 맞춰, 이런 기기의 대단한 성장이 여러 범위에서 지속되었다. 많은 개발자들은 모바일 기기의 GPS 칩, 가속측정기, 자이로스코프 등 특별한 특성에서 이점을 가지고 있다. 임베디드된 GPS 칩은 모바일 기기가 지구의 어디에 있는지 정확히 알려준다. 이 기능은 특히 네비게이션 앱을 디자인하는데 큰 도움이 된다. 가속도계는 모바일 기기가 땅을 기준으로 그것의 출처를 알려준다. 몇몇 컴퓨터 게임은 가속계를 지원하지만, 플레이어는 마우스나 키보드가 아닌 모바일 기기를 이용해서 이런 기능을 이용한다. 아마도, 이런 기능의 유용성은 AR 어플리케이션에도 보인다. 

이런 온라인 서비스를 제공하기 위해서, 모바일 기기는 IEEE standard 802.11또는 셀룰러 데이터 네트워크를 사용한다. 메모리 용량과 프로레싱 속도는 PC보다는 제한된다. 반면에 256GB의 저장소를 가진 스마트폰과 태블릿은 8TB의 PC와는 크게 다르다. 비슷하게, 전력 소보가 큰 문제인데, 모바일 기기는 더 작은 프로세서를 쓴다.

두개의 운영체제가 현재 모바일 시스템을 지배하고 있다. IOS는 애플 아이폰과 Ipad용이다. 안드로이드는 많은 기기를 지원한다. 이 두 시스템의 상세는 2장에서 보겠다.

### 1.10.3 클라이언트-서버 컴퓨팅

현대의 네트워크 구조는 클라이언트 시스템이 만들어낸 요구를 만족하는 서버 시스템의 나열이다. 이 특수화된 분산화 시스템의 형태는 **클라이언트-서버 시스템**이라고 한다.

서버 시스테은 컴퓨트 서버와 파일서버로 크게 카테고라이즈되어있다.

- 컴퓨터-서버 시스템은 어떤 클라이언트가 액션(데이터 읽기)을 수행할수 있는 인터페이스를 제공한다. 서버는 액션을 실행하고 결과를 클라이언트에게 보낸다. 데이터베이스를 실행하는 서버는 고객에게 데이터를 보낸다.

- 파일-서버 시스템은 클라이언트가 어디서 파일을 생성, 업데이트, 삭제를 할수 있는지 인터페이스를 제공한다. 이 시스템의 예시는 웹브라우저를 실행하는 고객에게 파일을 전달하는 웹서버이다. 실제 파일의 내용물은 다양하고, 전통적인 웹 페이지부터 고품질의 비디오 같은 풍부한 멀티미디어가 될 수도 있다.

### 1.10.4 피어 투 피어 컴퓨팅

다른 분산시스템 구조는 피어 투 피어 시스템 모델이다. 이 모델에서, 클라이언트와 서버는 구별되지 않는다. 대신에 모든 노드는 시스템 내에서 피어로 구별되며, 각각이 클라이언트이자 서버가 된다. 피어 투 피어 시스템은 전통적인 클라이언트-서버 시스템보다 큰 이점을 제공한다. 클라이언트-서버 시스템에서 서버는 병목현상에 빠진다. 하지만 피어2피어 시스템에서, 서비스는 네트워크를 통해 분산된 몇몇 시스템을 지원한다.

피어투피어 시스템에 참여하려면, 노드는 먼저 피어의 네트워크에 접속한다. 한번 네트워크에 참여하면, 그것은 서비스를 제공하고 다른 노드로부터 서비스를 요청 가능하다.

- 노드가 네트워크에 참여하면, 그것의 서비스는 네트워크의 중심화된 룩업 서비스에 등록된다. 특정 서비스를 원하는 어떠한 노드는 이 중앙화 룩업 서비스에 어떤 노드가 서비스를 제공하는지 확인하기 위해서 접속한다. 나머지 통신은 클라이언트와 서비스 제공자가 차지한다.

- 다른 방식은 아무런 룩업 방식을 사용하지 않는다. 대신에, 피어는 다른 노드에게 서비스를 네트워크의 위해 요청을 소문내서 클라이언트처럼 행동해야한다. 그 서비스를 제공하는 노드는 요청을 만든 피어에게 반응한다. 이런 방법은 *디스커버리 프로토콜*이 반드시 피어가 네트워크속 다른 피어가 제공하는 것을 허용하게 해준다.

피어2피어 네트워크는 1990년대에 몇몇 파일 공유 서비스에서 유명세를 탔다. 냅스터 시스템은 첫번째 타입과 비슷했다. 중앙화된 서버가 저장된 파일의 인덱스를 저장했고 교환은 피어노드간에 이루어졌다. 그누텔라 시스템은 2번째 방식과 비슷한 기술을 채택했다. 클라이언트가 노드 시스템에 파일을 찾아다녔고 그 서비스를 제공하는 노드는 요청에 반응했다. 피어2피어 네트워크는 저작권있는 물건을 익명으로 바꿀수 있었고, 이런 저작물의 분배를 제제하는 법이 있다. 냅스터는 곧장 법적 위반을 했고 그것의 시스템은 2001년에 멈췄다. 이런 이유때문에 파일의 교환은 여전히 불분명하다.

스카이프는 다른 예시의 피어투피어 컴퓨팅이다. 그것은 클라이언트 간에 통화와 영상통화를 **voice over IP(VoIP)**로 제공한다. 스카이프는 피어투피어 방식의 혼합된 접근이다. 그것은 중심화된 로그인 서버를 가지지만, 통신하는 유저간에 비 중심화된 방식도 포함되어있다.

### 1.10.5 클라우드 컴퓨팅

클라우드 컴퓨팅은 컴퓨팅, 저장소, 앱과 같은 서비스를 네트워크를 건너서 배달하는 것이다. 몇몇 방식에서, 가상화의 논리적 확장이다. 이 기능을 위해서는 가상화가 기초 되기 때문이다. 예를 들어 아마존 Elastic-Computer Cloud(ec2) 기능은 수천개의 서버와 수백만개의 가상머신과 페타 바이트의 저장소를 인터넷을 사용하는 누구에게나 제공한다. 유저는 이 리소스를 얼마나 사용하냐에 따라서 돈을 지불한다.

- 공공 클라우드 - 인터넷으로 접근 가능한 클라우드 서비스에 페이만하면 다 제공해준다.
- 사설 클라우드 - 특정 회사의 소유로 운영되는 클라우드
- 복합 클라우드 - 공공과 사설 클라우드 구성을 가진 클라우드
- Software as a Service(SaaS) - 인터넷을 통해 가능한 하나 또는 여러개의 애플리케이션(워드프로세서)
- Platform as a Service(PaaS) - 인터넷을 통해 사용가능한 애플리케이션의 소프트웨어 스택
- Infrastructures as a service(IaaS) - 인터넷에 전반적인 서버 또는 저장소

이런 클라우드 컴퓨팅은 명확하지 않고, 여러가지 환경이 다양한 타입의 혼합으로 제공된다.

확실히, 전통적인 운영체제는 많은 종류가 인프라 스트럭쳐는 기본적으로 지원한다. 이런 VMM은 자신의 툴을 이용해서 자신을 관리한다. 이런 툴들은 제공된 클라우드 자원을 관리하고 클라우드 구성요소에 대해서 컴포넌트를 제공하고, 새로운 타입의 운영체제를 고려한다.

### 1.10.6 Real-Time Embedded Systems

임베디드 컴퓨터들은 현존하는 가장 많은 컴퓨터이다. 이 기기들은 어디에서나 찾을수 있다. 그들은 특정한 기능을 한다. 이 시스템은 보통 원시적으로 만들어졌기에 운영체제는 제한된 기능을 지원한다. 보통, 그들은 작거나 아무런 인터페이스도 제공하지 않으며, 시간 모니터링과 하드웨어 기기 매니징에 많은 시간을 쓴다.

이러한 임베디드 시스템은 상당히 퍼져있다. 몇몇은 리눅스 같은 일반 목적 컴퓨터에서 일반 운영체제에서 작동한다. 다른 것들은 특정 목적의 임베디드 운영체제에서 기능이 바라는 것을 제공한다. 

임베디드 시스템의  사용은 점점 커지고 있다. 이런 기기의 힘은, 단일 유닛과 네트워크와 웹의 구성요소이다. 현재까지, 전체 집은 컴퓨터화되고 있고 난방, 불, 알람시스템, 커피메이커에 사용된다. 웹 엑세스는 집 소유자가 집에 도착하기전에 집에게 명령을 내릴수 있다. 언젠가는 냉장고가 우유가 떨어진 것을 알려줄 것이다.

임베디드 시스템은 항상 작동하는 리얼타임 운영체제이다. 이 리얼타임 시스템은 프로세서의 명령 또는 데이터의 흐름이 엄격한 시간 필요할때도 사용되지만, 그것은 정교한 앱에서 또한 사용된다. 센서는 컴퓨터에 데이터를 가져온다. 컴퓨터는 데이터를 분석하고 센서 인풋을 조절하는데 사용된다. 과학적인 실험을 조정하는 시스템은 리얼타임 시스템이다.(의료 이미징 시스템, 산업 조절  시스템) 몇몇 자동차 엔진 연료 주입 시스템, 집의 가구 컨트롤러, 무기 시스템 또한 리얼타임 시스템이다.

리얼 타임 시스템은 잘 정의된, 고정 시간 제어다. 프로세싱은 반드시 정의된 제한이다. 예를 들어, 로봇팔이 차를 만드는 중에 부수더라고 멈추라는 신호를 보내지 않는다. 리얼 타임 시스템은 오직 그것이 시간 제약에 맞게 정확한 결과를 내었을때 제대로 작동된 것이다. 전통적인 랩탑 시스템에 비해서 이것은 빠르게 반응된다.

## 1.11 Free and Open-Source Operating Systems

운영체제의 연구는 큰 수의 공짜 소프트웨어와 오픈 소스로 만들어졌다. 공짜 운영체제와 오픈소스 운영체제는 컴파일된 바이너리 코드보다 소스 코드 포맷이 존재했다. 무료 소프트웨어와 오픈 소스 소프트웨어는 두가지 그룹의 사람들끼리 싸웠다. 무료 소프트웨어는 소스 코드가 존재했을 뿐 아니라 재사용, 재분배, 사용하는데 비용이 없었다. 오픈 소스 소프트웨어는 이러한 라이센싱도 필요없었다. 그러므로 모든 무료 소프트웨어가 오픈소스이지만, 몇몇 오픈 소스 소프트웨어는 무료가 아니다. GNU/Linux는 가장 유명한 오픈 소스 소프트웨어이고 분배는 공짜이다. 마이크로 소프트 윈도우는 유명한 클로즈드 소스이다. 운도우는 재산 소프트웨어이다. 애플의 macOS 운영체제는 복합적이다. 그것은 다윈이라는 오픈소스 커널을 포함하지만 재산이고 클로즈드 소스 컴포넌트이다.

소스코드가 시스템에서 이용되는 이진 코드로 바뀌는 컷을 허용한다. 반대의 행동은 꽤나 일이 필요하고 코멘트같은 중요한 아이템은 절대로 복원되지 않는다. 운영체제를 배우는 것은 다른 이점이 있다. 소스코드와 함께라면 학생들은 운영체제를 바꾸고 바뀐점을 찾아낼수 있다. 이 교재는 운영체제 바꾸기를 포함한다. 하이레벨의 알고리즘을 포함한다. 이책을 통해서 우리는 더 깊은 연구를 위한 오픈소스 예시를 제공한다. 오픈 소스 운영체제에는 많은 장점이 있는데, 관심있는 프로그래머의 컴뮤니티를 포함한다. 여기서 코들르 도운사람, 디버그를한사람, 분석을 한사람을 찾을수 있다. 오픈 소스 코드는 클로즈드 소스 코드보다 보안보안하다. 왜냐하면 그 코드를 보는 사람이 많기 때문이다. 확실히 오픈 소스 코드는 버그를 가지지만, 오픈소스는 다양한 사람들이 사용하고 코드를 보면서 빠르게 고쳐진다. 회사는 그들의 프로그램을 파는 것으로 돈을 벌기에 코드를 오픈소스화 하는 것을 망설이지만, 레드햇과 많은 회사들은 오픈하는게 더 이득이라고 생각하기도 한다. 이익은 계약 지원과 소프트웨어가 돌아가는 하드웨어 판매로도 생성될 수 있다.

### 1.11.1 역사

현대 컴퓨팅에서, 소프트웨어는 소스코드였다. MIT의 원조 해커들은 그들의 프로그램을 다른 사람을 위해서 서랍장에 두었다. 한 유저 그룹은 그들의 미팅동안 코드를 교환했다. 한 기업은 테이프에 모아둔 소스코드 프로그램 기부를 1970년대에 관심있는 사람들에게 주었다. 

컴퓨터와 소프트웨어 회사들은 그들의 인증된 컴퓨터와 지불하는 고객에게만 사용을 제한했다. 컴파일된 이진파일을 뿌리는 것은, 소스코드 그자체보다는 훨씬 자신들의 코드를 지키고 아이디어르 지키는데 유용했다. 그들의 미팅동안 코드가 공유되었지만, 취미 머신을 위한 운영체제는 여전히 사유화되었다. 그렇게 1980년에 소프트웨어 사유화는 일반적이게 되었다.

### 1.11.2 무료 운영체제

소프트웨어 사용과 재분배 제한을 카운터하기 위한 움직임이 있었다. UNIX 운영체제에 비슷한 GNU 시스템을 리처드 스톨만이 무료로 개발했다. 물론 '무료'는 사용의 자유이지, 가격은 아니었다. 무료 소프트웨어 운동은 돈을 위해서 카피를 교환하는 것이 아니라 유저가 4가지 확실한 자유가 있었다. 
1. 프로그램 런은 마음껏.
2. 소스코드를 연구, 교환하고 카피를 팔거나 주는 것은
3. 변화를 주고 혹은
4. 주지 않는 것이다.
1985년에 스톨만은 GUN 매니페스토를 퍼블리시했고 소프트웨어는 공짜여야한다고 주장했다. 그는 또한 Free Software FOundation을 형성했고 무료 소프트웨어의 개발과 사용을 응원했다.
FSF 재단은 카피라이트 대신에 카피레프트를 개발했다. 자신의 작품을 카피레프팅은 누구에게나 그 작품의 카피를소유하게하고 4가지 필수 자유를 보장하는 것이었다. 


### 1.11.6 배움을 위한 오픈소스 시스템

무료 소프트웨어는 수많은 프로그래머가 오픈 소스 프로젝트를 만드는 군단이 되도록 이끌었다. 오픈 소스 프로젝트는 학생들이 배우는 도구의 소스 코드로서 사용이 되었다. 그들은 프로그램을 변경하고 테스트했고 버그를 발견하고 고치고 더 성숙하게 했다. 그 소스 코드의 실존은 멀틱스 같은 역사적인 프로젝트를 가능하게 만들었고 학생들이 그들의 프로젝트를 이해하고 새로운 프로젝트로서 이끌게 했다.

다른 오픈소스 운영체제의 장점은 다양성이다. GNU/LINUX와 BSD UNIX는 둘다 오픈소스 운영체제이고 그것의 고유 목적, 기능, 라이센싱이 있다. 가끔은 라이센스들은 독점적이고 교차로 생성한다. 예를 들어 몇몇 오픈소스 솔라리스의 메이저 구성요소는 BSD 유닉스이다. 무료 소프트웨어의 장점은 그 프로젝트의 질을 올려주고 많은 개인/기업 참여자를 만들어내는 것이다.