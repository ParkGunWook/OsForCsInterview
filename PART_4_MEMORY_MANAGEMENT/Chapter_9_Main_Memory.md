...
이번 절에서, 우리는 페이지 테이블을 건설하는 가장 일반적인 방법을 살펴보겠다. 계층적 페이지 테이블, 해쉬 페이지 테이블, 인버트 페이지 테이블이다.

### 9.4.1 계층적 페이징

대부분의 현대 컴퓨터 시스템은 거대한 논리 주소 공간을 지원한다.(2^32~2^64) 이런 환경에서, 페이지 테이블은 과도하게 커진감이 있다. 예를 들어서, 32비트 논리 주소공간을 고려하겠다. 만약에 시스템의 페이지 사이즈가 4KB(2^12)이면, 페이지 테이블은 2^20의 엄청난 엔트리들을 가질 수 있다. 각각의 엔트리가 4바이트를 포함하면 각 프로세스는 페이지테이블 자체로만 최대 4MB의 물리주소를 필요로한다. 확실히, 우리는 페이지 테이블을 메인메모리에 연속적으로 할당하고 싶지 않다. 페이지 테이블을 작은 조각으로 나누는 한가지 방법이 있다. 우리는 이 분할을 몇가지 방법으로 실현 가능하다.

한가지 방법은 2단 페이징 알고리즘을 이용하는 것인데, 페이지 테이블 자체가 페이지되는 것이다. 예를 들어서, 32비트 논리 주소 공간을 가진 시스템을 고려하고 페이지 크기가 4KB이다. 논리 주소는 20비트를 가진 페이지 넘버로 구성되고 페이지 오프셋이 12비트이다. 우리가 페이지 테이블을 페이지 하기 때문에, 페이지 너버는 다시 10비트 페이지 넘버와 10비트 페이지 오프셋으로 나뉜다.

한개의 바깥 페이지 넘버인 p1과 안쪽 페이지 넘버 p2를 가지게 된다. 주소 번역은 바깥에서 안으로 이루어지고 이러한 구조는 **foward-mapped** 페이지 테이블이라고 불린다.

64비트 논리 주소 공간을 가진 시스템은, 2단 페이징 구조에서 적절하지 않다. 예를 들어서, 4KB인 사이즈 페이지는 2^52의 엔트리를 가진다. 만약 우리가 2단 페이징 구조를 사용하면, 안쪽 페이지 테이블은 한 페이지의 길이가 되거나 2^10 4바이트 엔트리가 된다. 바깥 주소는 2^42엔트리, 2^44바이트가 될 것이다. 이런 큰 테이블을 피하는 한가지 명백하 방법은 바깥 페이지를 작은 조각으로 나누는 것이다.(이 방법은 32비트에서도 효율성과 유연성을 위해서 적용했다.)

우리는 외부 페이지 테이블을 다양하게 나눌 수 있다. 예를 들어서 3단계 구조로 나누겠다. 그렇게하면 32, 10, 10, 12와 같이 나눌 수 있게된다. 이렇게 해도 여전히 외부 페이지는 2^32로 벅차다. 즉 2^32개의 4바이트이므로 16GB의 크기의 엔트리를 가진다. 다음 방법은 4단계이고 더욱더 깊게 갈 것이다. 그래서 64비트 UltraSPARC는 7단계의 페이징이 필요하다. 엄두도 못낼 숫자의 메모리 엑세스가 각 논리 주소를 번역하기 위해서 필요한 것이다. 이런 이유때문에 64비트 구조에서는, 계층 페이지 테이블은 잘 이용되지 않는다.

### 9.4.2 해쉬 페이지 테이블

32비트보다 큰 주소 공간을 관리하는 한가지 방법은 **Hashd page table**이고, 해쉬 값이 가상 페이지 숫자가 된다. 각 해쉬 테이블의 엔트리는 같은 위치로 해쉬되는 원소이 링크드 리스트를 포함한다. 각 원소는 3가지 필드를 가진다. (1) 가상 페이지 숫자 (2) 매핑된 페이지 프레임의 값 (3) 링크드 리스트에서 연결된 다음 원소의 포인터

알고리즘은 다음과 같다. 가상 주소의 가상 페이지 넘버가 해시 테이블에 해시된다. 가상 페이지 넘버는 링크드 리스트의 첫번쨰 원소의 1번 필드와 비교된다. 만약 매칭되면, 적절한 페이지 프레임으로 넘어간다. 만약 매칭되지 않으면, 연속되는 링크드 리스트의 다음 원소를 찾는다. 이 구조의 변형은 64비트 주소 공간이 요청되고 유용해졌다. 변형은 **clustered page tables**를 사용하고, 해시 테이블의 각 엔트리가 단일 페이지보다는 여러 페이지를 해쉬드 페이지 테이블과 비슷하다. 그러므로, 싱글 페이지 테이블 엔트리는 다양한 물리 페이지 프레임에 매핑이 가능하다. 클러스터 페이지 테이블은 특히 **sparse(드문)** 주소 공간에 유용하고, 이런 주소 공간은 메모리 참조가 비연속적이고 주소공간에 흩날려있다.

### 9.4.3 Inverted 페이지 테이블

일반적으로, 각 프로세스는 관계형 페이지 테이블을 가지고 있다. 페이지 테이블은 각 페이지에 한개의 엔트리를 가지고 있다.(각 가상 주소를 위한 슬롯) 이 테이블 대표는 자연적이고, 프로세스가 페이지를 통해서 페이지의 가상 주소를 찾기 때문이다. 운영체제는 반드시 이 참조를 물리 주소로 바꾸어야한다. 테이블이 가상주소로는 정렬되어있기 때문에, 운영체제는 물리 주소 엔트리가 어디에 있는지 계산이 가능하고 그 값을 직접 쓸 수 있다. 이 방법의 한가지 단점은 각 페이지 테이블이 수백만의 엔트리를 가진다는 것이다. 이러한 테이블은 물리 메모리가 사용된 곳을 쫓느라 물리 메모리의 큰 부분을 소비하게된다.

이런 문제를 해결하기 위해서, 우리는 **Inverted page table**을 사용할 수 있다. 인버티드 페이지 테이블은 메모리의 각 페이지(프레임)에 한개의 엔트리만 존재하는 것이다. 각 엔트리는 페이지의 실제 메모리 공간이 저장된 가상 주소를 포함하고, 어떤 프로세스가 페이지를 가졌는지 정보를 알려준다. 그러므로 오직 한개의 페이지 테이블이 시스템에 존재하고, 물리메모리의 각 페이지는 하나의 엔트리만 가진다. 인버티드 페이지 테이블은 주소 공간 식별자를 각 페이지 테이블의 엔트리에 저장해둔다. 그래야지 여러가지 다른 주소 공간을 물리 메모리에 매핑할 수 있기 때문이다. 주소 공간 식별자를 저장하는 것은 특정 프로세스의 논리 주소 공간이 물리 페이지 프레임에 매핑된걸 확신하게 한다. 이런 시스템의 에시는 64비트 UltraSPARC와 PowerPC이다.

이 방법을 설명하기 위해서, 우리는 IBM RT에서 사용된 인버티드 페이지 테이블을 보겠다. IBM은 인버티드 페이지 테이블을 처음 사용한 대기업이다. IBM RT에서, 각 주소 공간은 3가지를 구성하는데, <process-id, page-number, offset>이다.

각 인버티드 페이지 테이블 엔트리는 페어인 <process-id, page-number>인데, process-id는 주소 공간 식별자의 역할을 가정한다. 메모리 참조가 일어나면, 가상 주소의 부분이 메모리 서브시스템에 보여진다. 인버티드 페이지 테이블은 매치되는 것을 찾기 시작한다. 만약 매치가 되면, 엔트리 i는 물리주소 <i, offset>이 된다. 만약 매치가 찾아지지 않으면, 잘못된 주소 접근이라고 한다.

비록 이 구조가 각 페이지 테이블을 저장하는 메모리의 양은 줄였지만, 페이지 레퍼런스가 일어날때의 검색 시간은 증가시켰다. 인버티드 페이지는 물리 페이지로 정렬되어있는데 반해서, 룩업은 가상메모리로 이루어진다. 이것은 매치가 일어나기 전까지 모든 테이블을 살펴보아야할 수도있다. 이 검색은 꽤 긴 시간이 걸린다. 이 문제를 경감시키기 위해서, 우리는 해시 테이블을 이용해서 페이지 테이블을 제한한다. 물론 각 해시 테이블은 메모리 참조 진행하고, 한가지 가상 메모리 레퍼런스는 2번의 메모리 읽기가 필요한데, 하나는 해시 테이블 엔트리와 하나는 페이지 테이블을 위해서이다.

한가지 흥미로운 이슈는 공유된 메모리에 관한 것이다. 일반적인 페이징에서, 각 프로세스는 그것의 페이지 테이블이 있어서 다양한 가상 메모리가 같은 물리주소에 매핑될 수 있었다. 이 메서드는 인버티드 페이지에서는 불가능하다. 왜냐하면 하나의 가상주소만이 모든 물리 페이지에 존재하고, 하나의 물리 페이지는 2개의 가상메모리 공간을 가질 수 없다. 그러므로, 인버티드 페이지 테이블에서, 오직 한가지 가상주소의 물리주소 매핑이 가능하다. 다른 프로세스와 공유하는 레퍼런스는 페이지 폴트를 일으키고 다른 가상주소로 매핑이 교체될 것이다.

### 9.4.4 Oracle SPARC Solaris

**Solaris**는 **SPARC** CPU에서 64비트 운영체제이고 다중 레벨 페이지 테이블을 유지하지 않으면서 가상 메모리의 문제를 해결했다. 그것읜 접근은 비트 컴플렉스이지만, 해시 페이지 테이블을 이용해서 효율적으로 문제를 해결했다. 두가지 해시 테이블이 있다.(커널, 유저 프로세스) 각갂은 매모리 주소를 가상에서 물리 주소로 매핑한다. 각 해시 테이블 엔트리는 매핑된 가상 메모리의 연속적인 공간이고, 각 페이지 별로 분리된 해시테이블 엔트리보다 낫다. 각 엔트리는 베이스 주소와 엔트리가 대표하는 페이지의 숫자를 가르키는 폭이 있다.

가상에서 물리로의 번역은 만약 각 주소가 해시 테이블로 찾아지면 많은 시간을 소요할 것인데, 그래서 CPU는 translation table entries(TTEs)를 가지고 있는 TLB를 하드웨어 룩업으로 구현했다. TTE의 캐시는 Translation storage buffer(TSB)에 저장되고, 최근에 접근한 페이지를 저장한다. 가상 메모리 참고가 일어나면, 하드웨어는 TLB에서 검색한다. 만약 실패하면 하드웨어는 적합한 것을 찾기위해서 TSB로 들어간다. 이 **TLB walk** 기능은 현대의 CPU에서 많이 이용된다. 만약 TSB에서 찾으면, CPU는 TSB 엔트리를 TLB에 복사하고 메모리 번역이 완료된다. 만약 TSB에서 못찾으면, 커널이 해시테이블을 검색하기 위해서 인터럽트된다. 커널은 적절한 해시 테이블로부터 TTE를 생성하고 CPU MMU가 그것을 TSB에 저장해서 TLB에까지 자동으로 로딩된다. 마침내, 인터럽트 핸들러가 컨트롤을 MMU에 돌려주고, 주소 번역을 마치고 메인메모리로 부터 요청한 바이트또는 워드를 되찾아온다.

## 9.5 스와핑

그들이 실행하는 프로세스 명령어와 데이터들은 반드시 실행되기 위해서 메모리에 있어야한다. 그러나, 프로세스 또는 프로세스의 일부분은 임시적으로 **backing sotre**인 메모리 바깥으로 **swapped**될 수 있고 연속될 실행을 위해서 메모리로 돌아올 수 있다. 스와핑은 모든 프로세스의 시스템의 전체 물리 주소 공간을 초과하는 전체 물리 주소 공간에서 가능하다. 그러므로 시스템에서의 멀티 프로그래밍의 증가 때문이다.

### 9.5.1 표준 스와핑

표준 스와핑은 전체 프로세스가 메인메모리와 배킹 스토어 사이를 움직이는 것이다. 배킹 스토어는 보통 빠른 2차 저장소이다. 그것은 반드시 저장되거나 복구되어야할 필요가 있는 프로세스의 일부분을 거주시켜야할만큼 커야한다. 프로세스 또는 부분이 배킹 스토어에 스왑되면, 프로세스와 관련된 데이터 구조는 반드시 배킹스토어에 쓰여야한다. 멀티스레드 프로세스에서, 모든 스레드 데이터 구조는 스왑되어야한다. 운영체제는 반드시 프로세스를 위한 스왑 아웃된 메타데이터를 유지해야한다. 그래서 그들은 그들이 메모리에 돌아왔을때 복구될 수 있다.

표준 스와핑의 장점은 물리 메모리가 추가로 구독될수 있는 것인데, 그래서 시스템은 실제의 물리 메모리보다 많은 프로세스를 처리할 수 있다. 유휴거나 거의 유휴인 프로세스들은 스와핑을 위해서 좋은 후보들이다. 비활성화된 프로세스가 할당된 메모리는 실행중인 프로세스에 헌신할 수 있다. 만약 스왑 아웃된 비활성 프로세스가 다시 활성화되면, 그것은 반드시 스왑인 되어야한다.

### 9.5.2 페이징을 동반한 스와핑

표준 스와핑은 예전 UNIX 시스템에서 주로 쓰였는데, 그것은 현대 운영체제에서는 더이상 사용되지 않는데, 프로세스를 메모리와 배킹 스토어를 이동시키는 시간의 양이 엄청나기 때문이다. (솔라리스는 여전히 표준 스와핑을 사용하는데, 오직 유효한 메모리가 매우 적은 극한의 상황에서만이다.)

윈도우와 리눅스를 포함한, 대부분의 시스템에서는 이제 전체 프로세스보다느 프로세스의 페이지를 스와핑하는 변형을 사용한다. 이 전략은 여전히 물리 메모리를 다중으로 쓸 수 있게하지만, 전체 프로세스의 스와핑 비용을 발생시키지 않는데, 오직 작은 양의 페이지만이 스와핑된다. 실제로, *swapping*이라는 용어는 이제 표준 스와핑을 의미하고, 페이징이 스와핑을 동반한 페이징이라고 의미한다. **page out** 명령어는 페이지를 메모리에서 배킹 메모리로 이동시키고, 반대는 **page in**이라고 한다. 10장에서 볼 것인데, 스와핑-페이징은 가상 메모리와 합이 잘 맞는다.

### 9.5.3 모바일 시스템에서의 스와핑

PC와 서버 같은 대부분의 운영체제는 스와핑 페이지를 지원한다. 반대로, 모바일 시스템은 스와핑을 전혀 지원하지 않는다. 모바일 기기는 더 큰 비휘발성 저장공간인 하드디스크보다는 플래시 메모리를 사용한다. 결과적인 공간은 왜 모바일 운영체제 디자이너가 스와핑을 피하는 이유가 된다. 다른 이유는 메인메모리와 플래시 메모리 사이의 믿을 수 없어지고 낮은 산출량이전에 플래시 메모리가 견딜 수 있는 쓰기의 제한된 수를 포함한다.

스와핑 대신에, 빈 메모리가 일부 스레시 홀드 아래로 떨어지면, 애플의 iOS는 앱들에게 자발적으로 할당된 메모리를 버리라고 한다. 읽기 전용 데이터들은 메인 메모리에서 제거되고 후에 만약 필요해지면 플래시 메모리로부터 재 로드된다. 그러나, 충분한 메모리를 비우는데 실패하면, 운영체제가 직접 종료시킨다.

안드로이드는 iOS와 비슷한 전략을 선택했다. 그것은 만약 여유메모리가 충분하지 않으면 프로세스를 종료한다. 그러나, 프로세스를 종료하기 이전에, 안드로이드는 **application state**를 플래시 메모리에 작성하고 빠르게 재시작된다.

이런 제한때문에, 모바일 시스템을 위한 개발자들은 반드시 조심스럽게 메모리를 할당하고 해제해서 그들의 앱들이 과도한 메모리또는 메모리 누수를 피하도록 해야한다.

## 9.6 인텔-32 와 64 비트 아키텍처

인텔칩 아키텍처는 몇십년간 개인 컴퓨터 시장을 지배했다. 16비트 인텔 8086은 1970년도에 나타났고 다른 버전의 칩들도 나왔다. 인텔은 후에 32비트 칩(IA-32)를 펜티움 프로세서의 패밀리로 포함했다. 최근에는 x84-64 아키텍처에 기초한 64비트 칩들이 나오고 있다. 요즘에, 대부분의 인기 있는 PC운영체제들은 인텔 칩에서 작동한다. 인텔의 독점은 모바일 시스템으로는 닿지 않았고 ARM 아키텍처가 상당한 성공을 얻었다.

이 절에서, 우리는 IA-32와 x86-64 아키텍처에서의 주소 번역을 설명하겠다. 우리가 시작하기에 앞서서, 인텔은 몇가지 다양한 버전의 아키텍처를 몇년간 발매했고, 우리는 그 칩들의 정확한 메모리 관리 구조 상세는 제공하지 않는다. 우리는 인텔 CPU의 주요한 메모리 관리 개념을 보여주겠다.

### 9.6.1 IA-32 아키텍처
IA-32 시스템에서의 메모리 관리는 두가지 요소로 나뉜다. 세그멘테이션과 페이징이다. CPU는 논리 주소를 생성하는데, 세그멘테이션 유닛에 의해서 주어진다. 세그멘테이션 유닛은 각 논리 주소를 위한 선형 주소를 생성한다. 선형 주소는 페이징 유닛에 주어지고 메인메모리의 물리주소를 생성한다. 그러므로, 세그멘테이션과 페이징 유닛은 같은 MMU(memory management unit)으로 구성된다.

#### 9.6.1.1 IA-32 세그멘테이션

IA-32 구조는 세그먼트를 4GB까지 허용한다. 그리고 프로세스별 세그먼트의 최대수를 16K로 제한한다. 프로세스의 논리 주소공간은 두가지로 나뉜다. 첫번째 파티션은 프로세스에 프라이빗한 최대 8K의 세그먼트로 이루어진다. 두번째 파티션은 모든 프로세스간에 공유하는 최대 8K의 세그먼트이다. 첫번째 파티션에 대한 정보는 **local descriptor table(LDT)**에 저장되고 두번째 파티션은 **global descriptor table(GDT)**에 구성된다. 특정 세그먼트의 상세 정보로 기술된 8바이트 세그먼트 기술어로 구성된 LDT와 GDT의 각 엔트리는 세그먼트의 베이스와 리미트 주소가 포함되어있다.

논리 주소는 16비트로 이루어진 셀렉터는 한 쌍(selector, offset)을 이룬다.
`13 s / 1 g / 2 p`
셀렉터에서 s는 세그먼트 숫자를 지정하고 g는 GDT인지 LDT인지, p는 보호를 의미한다. 나머지 16비트인 오프셋은 세그먼트의 위치를 알려주는 바이트이다.

머신은 6가지 세그먼트 레지스터를 가지는데, 6가지 세그먼트는 프로세스에 의해서 한번에 접근하게 허용한다. 그것은 또한 적절한 기술어가 LDT또는 GDT로부터 잡고있는 6개의 8바이트 마이크로프로그램 레지스터를 가진다. 이 캐시는 펜티움이 모든 메모리로 부터 기술어를 읽지 않도록 한다.

IA-32에서의 선형 주소는 32비트이고 다음과 같이 구성된다. 세그먼트 레지스터가 LDT 또는 GDT인 적절한 엔트리를 가르킨다. 세그먼트의 베이스와 리미트 정보가 **linear address**를 생성하기 위해서 사용된다. 먼저, 리미트는 주소 유효성을 확인한다. 만약 주소가 유효하지 않으면, 메모리 폴트가 생성된다. 만약 유효하면, 오프셋의 값이 베이스에서 더해지고 32비트 선형 주소를 만든다. 다음으로 우리는 어떻게 페이징 유닛이 선형 주소를 물리주소로 바꾸는지 보겠다.

#### 9.6.1.2 IA-32 페이징

IA-32 아키텍처는 페이지 크기를 4KB또는 4MB둘다 가능하게 한다. 4KB 페이지에서, IA-32는 두 단계 페이징 구조를 사용한다.(10,10,12). 주소 번역 구조는 계층적 페이징과 비슷하다. 대신에 높은 계층의 페이지 테이블을 여기서는 **page director**라고한다.(2^10) 여기서 안쪽 페이지 디렉토리가 10bit로 구성되어있는데 4MB는 이 숫자들 또한 offset으로 사용한다.

물리 메모리의 효율성을 향상시키기 위해서, IA-32 페이지 테이블은 디스크로 스왑될 수 있다. 이런 경우에, 페이지 디렉토리 엔트리에서 invalid bit가 사용되서 테이블이 현재 메모리를 가르키는지 디스크를 가르키는지 알린다. 만약 테이블이 디스크에 있으면, 운영체제는 다른 31개 비트를 테이블의 디스크 위치를 찾는데 사용한다. 테이블은 그러면 다시 수요에 맞게 메모리로 돌아온다.

소프트웨어 개발자가 32비트 아키텍처에서 4-GB 메모리의 제한을 찾아내고, 인텔은 **page address extension(PAE)**를 채택해서, 32비트 프로세서가 4GB보다 큰 물리 주소를 접근하게 해주었다. PAE 지원으로 생긴 근본적인 차이점은 페이징이 2단계 구조에서 3단 구조로 변한 것이다. 최 상단의 2개 비트가 **page directory pointer table**인 것이다.

PAE는 또한 페이지 디렉토리와 페이지 테이블 엔트리를 32비트에서 64비트로 늘렸고, 페이지 테이블의 베이스 주소와 페이지 프레임의 기본을 20에서 24비트로 늘렸다. 12비트 오프셋과 합쳐져서, PAE 지원은 주소 공간을 36비트로 늘렸고 64GB의 물리적 메모리 지원을 한다. 운영체제가 PAE를 사용하는 것은 중요하다. 리눅스와 맥OS또한 PAE를 지원한다. 그러나 32비트의 윈도우느느 여전히 4GB의 물리 메모리만 지원한다. 아무리 PAE가 켜져있어도.

### 9.6.2 x86-64

인텔은 64비트 아키텍처 개발에 흥미로운 역사가 있다. 그것의 첫번째 엔트리는 IA-64(**Itanium**)아키텍처인데, 그러나 이 아키텍처는 채택되지 못했다. 반면에, 다른 칩 제작사인 AMD는 x84-64라는 64비트 아키텍처를 IA-32 명령어 셋에 기반해서 확장하는 방식으로 만들었다. x84-64는 많은 논리와 물리 주소 공간을 지원했고, 다른 아키텍처적인 진보도 이루어냈다. 역사적으로, 암드는 인텔의 아키텍처 기반으로 개발했는데, 그러나 이제 인텔이 암드의 x86-64를 채택한다. 이 아키텍처에 대해서 논의하면 **AMD64**와 **INTEL64**같은 상업적인 이름이 아닌 일반적인 용어 **x84-64**를 사용하겠다.

64비트 주소공간이 2^64바이트의 메모리를 만들어낸다. 그러나 64비트 시스템이 이런 많은 주소를 가르킬 수 있음에도, 실전에서는 64비트보다 훨씬 적은 비트들이 사용된다. x84-64 아키텍처는 48비트 가상 주소를 페이지 크기 4KB,2MB, 1GB를 4레벨의 페이징 계층으로 지원한다. 선형 주소는 다음과 같이 나타난다.
|63-48|47-39(9bit)|38-30(9bit)|29-21(9bit)|20-12(9bit)|11-0(12bit)|
|---|---|---|---|---|---|
|unused|page map level4|page directory pointer table|page directory|page table|offset|

이 주소구조는 48비트의 가상 주소를 가지지만, PAE를 동반하면 52비트로서(4096테라바이트) 사용할 수 있다.

## 9.7 ARMv8 아키텍처

비록 인텔 칩이 개인 컴퓨터 시장을 30년간 독점했지만, 모바일 기기를 위한 칩은 ARM 프로세서가 대신 사용되고 있다. 흥미롭게도, 인텔은 칩 디자인과 생산을 했지만, ARM은 오직 디자인만 한다. 그들의 라이선스된 디자인은 칩 생산자에게 전해진다. 애플은 iPhone과 iPad 모바일 기기를 위한 ARM 디자인을 허가받았고, 대부분의 안드로이드도 마찬가지이다. 모바일 기기에 덧붙여서, ARM은 또한 리얼타임 임베디드 시스템을 위한 아키텍처 디자인도 제공한다. 100 billion ARM 프로세서가 생산되었고, 더욱 많이 사용될 것이다. 이 절에서, 우리는 64비트 ARMv8 아키텍처를 설명하겠다.

ARMv8은 3가지 **translation granules**를 가진다. 4KB, 16KB, 64KB이다. 각각의 번역 그래뉼은 다른 페이지 크기를 제공하는데, **region**이라고도 불리는 연속적 메모리의 큰 섹션이다. 페이지와 리전 사이즈는 아래와 같이 구분된다.
...
4KB와 16KB의 그래뉼은 4 레벨의 페이징이 사용된다. 64KB 그래뉼은 3레벨이 필요하다. 

4레벨의 계층적 페이징 구조는 다음과 같다.(TTBR 레지스터는 **translation table base register**이고 현재 스레드의 레벨 0 테이블을 가르킨다.)

|63-48|47-39(9bit)|38-30(9bit)|29-21(9bit)|20-12(9bit)|11-0(12bit)|
|---|---|---|---|---|---|
|unused|level 0 Index|level 1 Index|level 2 Index|level 3 Index|offset|

만약 4개의 레벨이 모두 사용되면 오프셋은 4KB 페이지 크기를 의미한다. 레벨 1과 레벨 2의 테이블 엔트리는 다른 테이블 또는 1GB region(level1 table)또는 2MB region(level2 table)을 가질 수도 있다. 예시로서, 만약 레벨 1 테이블이 1GB 리전을 level-2 테이블보다 언급하면, 낮은 30비트(0~29)는 2MB 리전의 오프셋이다. 

암 아키텍처는 또한 두단계의 TLB를 제공한다. 두개의 **micro TLBs**가 안쪽 레벨에 있고, 데이터를 위한 것과 명령어를 위한 것이다. 마이크로 TLB는 ASID를 또한 지원한다. 바깥 레벨은 단일 **maing TLB**이다. 주소 번역은 마이크로 TLB레벨에서 일어난다. 미스가 일어나면 main TLB가 체크된다. 둘다 실패하면 페이지 테이블을 쭉 살펴볼 것이다.