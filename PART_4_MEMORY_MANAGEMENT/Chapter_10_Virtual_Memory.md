...
페이징의 대안은 **memory compression**이다. 여기서, 수정된 프레임을 스페이스 스왑하기 위해서 페이징하는 것보다는, 우리는 몇가지 프레임을 한개의 프레임으로 압축하고, 시스템이 페이지 스와핑에 의지하지 않고 메모리 사용량을 감소하게 한다.

6개의 여유 프레임이 있고 이 숫자보다 떨어지면 페이지 교체를 이룬다고 생각하겠다. 교체 알고리즘은 4개의 프레임을 선택하고 프리 프레임 리스트에 둘것이다. 우선 이 프레임들은 수정된 프레임 리스트에 둔다. 일반적으로, 수정된 프레임 리스트는 스왑 공간에 내용을 쓰고나서, 프레임은 프리 프레임 리스트에 들어간다. 다른 대안은 프레임의 수를 압축하는 것이다.

우선 여유 프레임 리스트에서 하나를 삭제하고, 그 안에 수정된 프레임을 압축해서 쓴다. 그리고 이 것은 압축된 프레임의 리스트에 들어간다. 그리고 압축된 프레임들은 여유 프레임 리스트에 들어간다. 만약 압축된 프레임중 하나가 참조되면, 페이지 폴트가 일어나고, 압축된 프레임이 압축이 해제되고 메모리에 들어가게 된다.

모바일 시스템은 일반적인 스와핑또는 스와핑 페이지를 지원하지 않는다. 그러므로, 메모리 압축은 모바일 시스템에서 메모리 관리 전략을 위한 필수적인 요소중 하나이다. 윈도우와 맥OS 또한 메모리 압축을 지원한다. 윈도우 10에서는 마이크로 소프트는 **Universal Windows Platform** 아키텍저를 사용하는데, 모바일 기기를 포함한 윈도우 10에서 작동가능한 앱 플랫폼을 제공한다. 모바일 기기에서 작동하는 UWP 앱들은 메모리 압축을 위한 후보자들이다. 맥에서는 버전 10.9에서 메모리 압축을 지원했고, 여유 메모리가 부족할때 LRU 페이지를 압축하고 그것이 문제를 해결 못하면 페이징을 하였다. 이 압축기술은 SSD를 사용하는 랩탑장치에서도 스와핑보다 더 빠른 성능을 보였다.

비록 메모리 압축이 압축된 페이지를 여유 프레임에 할당하여도, 메모리의 상당한 절약이 압축 알고리즘에 따라서 있다. 데이터 압축의 한 형태로, 압축에 걸리는 시간과 압축의 정도에 따른 논쟁이 있다. 일반적으로, 높은 압축률은 느리고, 연산이 비싼 알고리즘이다. 현재 쓰이는 알고리즘은 보통 두 요소사이의 절충안이다. 추가로, 압축 알고리즘은 다양한 컴퓨팅 코어의 장점과 압축이 병렬로 실행해서 현재는 더욱 발전했다. 예를 들어서, 마이크로 소프트의 Xpress와 애플의 WKdm은 빠르고 30~50의 크기로 압축이 가능하다고 한다.

## 10.8 커널 메모리 할당

프로세스가 유저 모드에서 작동하고 추가적인 메모리를 요청할 때, 페이지들은 커널에 의해서 유지되는 여유 페이지의 리스트로부터 할당받는다. 이 리스트는 10.4에서 말한 페이지 교체 알고리즘을 사용하고 물리 메모리에 널리 퍼진 여유 메모리를 포함한다. 만약 유저 프로세스가 단일 바이트를 요구하면, 내부 단편화가 일어나고, 프로세스는 전체 페이지 프레임을 승인한다.

커널 메모리는 보통 유저 모드 프로세스를 만족시키기 위한 리스트와 다른 여유 메모리 풀에서 할당받는다. 여기에는 두가지 이유가 있다.

1. 커널은 변화하는 사이즈의 데이터 구조를 요청하고, 몇몇은 페이지 사이즈보다 작다. 결과적으로 커널은 메모리를 아껴쎠야하고 단편화로 인한 낭비를 줄여야한다. 이것은 운영체제가 커널 코드 또는 데이터가 페이징 시스템에 종속되어있지 않기 때문에 매우 중요하다.
2. 유저 모드 프로세스에 할당된 페이지들은 연속적인 물리 메모리를 가질 필요가 없다. 그러나, 일부 하드웨어 디바이스는 물리 메모리와 직접 상호작용하고, 가상 메모리 인터페이스의 장점이 없어진다. 그래서 결과적으로 물리적으로 연속적인 페이지에 상주해야한다.

따라오는 절에서, 우리는 커널 프로세스에 할당된 여유 메모리를 관리하기 위한 두가지 전략을 보겠다. "buddy system"과 slab allocation이다.

### 10.8.1 버디 시스템

버디 시스템은 물리적으로 연속적인 페이지에 고정된 사이즈의 세그먼트로부터 메모리를 할당한다. 메모리는 **power of 2 allocator**의 구역을 사용하고, 특정 기본 단위의 2의 지수를 제공한다.(4KB, 8KB, 16KB). 적절한 사이즈를 찾지 못한 것은 2의 지수승으로 올림된다. 예를 들어서 11KB는 16KB이다.

간단한 예시를 들어보자. 메모리 세그먼트의 사이즈가 초기에 256KB이고 커널은 21KB를 요청했다. 세그먼트는 두개의 **buddies**로 나누어지고, A_L과 A_R로 나누어서 128KB씩을 가진다. 이중 하나는 다시 64KB로 나누어지고 또다시 32로 나누어 질 것이다. 그래서 하나의 버디는 21KB를 만족시키기 위해서 사용된다.

버디 시스템의 장점은 얼마나 근접한 버디가 큰 구역을 얻기위해 합쳐지는 기술인 **coalescing**이다. 예를 들어서, 커널이 32유닛을 해제하면, 시스템은 32를 두개 coalesce하고 64를 합치고 128을 합쳐서 다시 256KB를 가진다.

버디시스템의 명백한 단점은 할당된 구역에 내부단편화가 심하다는 것이다. 예를 들어서, 33KB는 64KB로 만족이 된다. 실제로, 우리는 내부 단편화 때문에 50퍼센트의 할당유닛 이하가 낭비된다는 것을 보장할 수가 없다. 따라오는 절에서, 우리는 다음절에서 단편화로 인한 메모리 할당 낭비가 없는 구조를 보겠다.

### 10.8.2 Slab Allocation

커널 메모리 할당을 위한 두번째 전략은 **slab allocation**이다. **slab**은 하나 또는 여러개의 물리적으로 연속된 페이지로 만들어진다. **cache**가 하나 또는 여러개의 슬랩으로 구성된다. 예를 들어 프로세스 기줄자를 대표하는 데이터구조의 분리된 캐시, 파일 오브젝트의 분리된 캐시, 세마포의 분리된 캐시같은 각 유일한 데이터 구조를 가지는 단일 캐시가 있다. 각 캐시에는 캐시가 대표하는 커널 데이터 구조의 인스터스화된 **object**가 존재한다. 예를 들어서, 캐시가 세마포 오브젝트의 인스턴스를 보관하고, 프로세스 기술자 객체의 인스턴스를 저장하면 프로세스 기술자를 의미한다. 슬랩, 캐시, 객체의 관계가 있다. 

슬랩 할당 알고리즘은 커널 객체를 저장하는데 캐시를 사용한다. 캐시가 생성되면, `free`로 초기화된 객체들은 캐시에 할당된다. 캐시에 있는 객체의 수는 관련된 슬랩의 크기에 의존한다. 예를 들어서 12KB 슬랩(3개의 4KB의 연속 페이지)는 6개의 2KB 객체를 저장한다. 처음에는, 캐시의 모든 객체들은 `free`로 마크되어있다. 커널 데이터 구조를 위한 새로운 객체가 필요하면, 할당자는 요청을 만족하기 위해서 캐시로부터 free 객체를 할당할수 있다. 그 객체는 이제 캐시에 `used`로 마크된다.

커널 메모리가 슬랩 할당자로부터 프로세스 기술자를 대표하는 객체를 요청하는 시나리오를 살펴보자. 리눅스 시스템에서, 프로세스 기술자는 `struct task_struct`이고 1.7KB를 정확히 필요로 한다. 리눅스 커널이 새로운 태스크를 만들면, 그것은 캐시에 있는 `struct task_struct` 객체로부터 메모리를 요청한다. 캐시는 슬랩에 이미 할당되어 있고 free로 마킹된 `struct task_struct`를 슬랩에 할당한다.

리눅스에서 슬랩은 다음과 같은 상태를 가진다. 
1. Full. 슬랩의 모든 객체가 used이다.
2. Empty. 슬랩의 모든 객체가 free이다.
3. Partial 슬랩은 used와 free를 둘다 가진다.

슬랩 할당자는 먼저 부분 슬랩에서 여유 객체를 이용해서 요청을 만족하려고 한다. 만약 존재하지 않으면, free 객체가 빈 슬랩에서 할당된다. 만약 빈 슬랩이 없으면, 연속된 페이지에서 새로운 슬랩이 할당되고 캐시에 할당된다. 이 슬랩에서 객체 메모리는 할당된다.

슬랩은 두가지 주요한 장점을 제공한다.
1. 단편화로 인한 메모리 낭비가 없다. 단편화는 각 고유한 커널 데이터 구조가 관련된 캐시를 가지고, 각 캐시가 객체의 사이즈에 맞게 나누어진 하나 또는 여러개의 슬랩으로 이루어져 있기 때문에 일어나지 않는다. 그러므로, 커널이 객체를 위한 메모리를 요구하면, 슬랩은 필요한 메모리의 정확한 양을 리턴한다.
2. 메모리 요청은 빠르게 이루어진다. 슬랩 할당 구조는 객체가 자주 할당되고 해제 될때, 특히 효율적이다.(커널이 보통 이렇다.) 메모리를 할당하고 해제하는 행동은 시간을 잡아먹는 일이다. 더 나아가, 커널이 객체를 끝내고 해제하면, 그것은 free로 마킹되로 그것의 캐시로 돌아간다. 그러므로 커널로부터 다음 요청을 당장 사용가능하게한다.

슬랩은 솔라리스 2.4 커널에서 처음 나타났다. 그것의 범용성 목적 환경때문에, 이 할당자는 일부 유저모드 메모리에서도 사용되었다. 리눅스는 버디 시스템을 사용했는데, 2.2 버전에서 리눅스 또한 슬랩 할당자를 채택했다. 리눅스는 slab의 구현을 SLAB라고 했다. 리눅스의 현대 배포판은 SLOB과 SLUB 할당자라는 두가지 커널 메모리 할당자가 있다.

SLOB 할당자는 임베디드 시스템같은 제한된 메모리의 시스템을 위해 디자인 되었다. SLOB(simple list of blocks)는 객체의 3가지 리스트를 유지한다. *small*(256bytes이하), *medium*(1024bytes이하), *large*(나머지). 메모리 요청은 first-fit 정책을 통해서 적절한 리스트를 찾아서 할당한다.

2.6.24에서, SLUB 할당자는 SLAB을 기본 리눅스 커널 할당자로 대체했다. SLUB는 SLAB 할당자가 필요한 간접비를 대폭 감소시켰다. 예를 들어서, SLAB이 각 slab에 메타데이터를 저장하는데 비해서, SLUB은 리눅스가 사용하는 페이지 구조 `page`에 저장한다. 추가적으로 SLUB은 SLAB 할당자가 각 캐시의 객체를 유지하기 위한 CPU 큐를 포함하지 않는다. 많은 프로세스를 가진 시스템에서, 이 큐에 할당된 메모리의 양은 상당하다. 그러므로, SLUB은 시스템에서 작동하는 프로세서의 수에 맞게 더 좋은 성능을 제공한다.


## 10.9 Other Considerations

페이징의 가장 주요한 결정은 교체 알고리즘과 할당 정책이다. 그러나 몇가지 논의할 점이 더 있다.

### 10.9.1 Prepaging

순수한 디맨드 페이징의 명백한 성질은 프로세스가 시작할때 많은 페이지 폴트가 일어나는 것이다. 이 상황은 메모리의 첫 로컬리티를 얻으면서 생긴다. **Prepaging**은 이 높은 단계의 페이징을 예방한다. 이 전략은 메모리에 필요할 페이지의 일부 또는 전체를 한번에 가지고 오는 것이다.

워킹 셋 모델을 사용하는 시스템에서, 우리는 각 프로세스마다 워킹 셋 안에 있는 페이지의 리스트를 유지할 수 있다. 만약 우리가 프로세스를 중단해야하면, 우리는 프로세스의 워킹 셋을 기억한다. 프로세스가 재시작하면(I/O가 종료되거나 충분한 여유 프레임이 가용해질때), 우리는 프로세스가 재시작하기 전에 자동으로 전체 워킹 셋을 메모리에 가져온다.

프리페이징은 몇가지 경우에 이점을 제공한다. 질문은 프리페이징의 비용이 상응하는 페이지 폴트를 서비스하는 것보다 비용이 적은지 확인하는 것이다. 프리페이징에 의해서 다시 가지고온 페이지가 사용되지 않는 것도 확인해야한다.

s의 페이지들이 프리페이징되었고 실제로는 s중에서 a(0~1)만이 쓰인다고 생각하자. 이 질문은 s*a의 페이지 폴트 절약이 큰지 s\*(1-a)의 불필요 페이지가 적은지이다. 만약 a가 0에 가까우면, 프리페이징은 패배하고, a가 1에 가까우면 프리 페이징이 이긴다.

그리고 실행프로그램에 프리페이징하는 것은 어렵다. 어떤 페이지를 다시 들고와야할지 불분명하기 때문이다. 파일을 프리페이징하는 것은 예측이 가능한데, 파일은 보통 연속적으로 엑세스되기 때문이다. 리눅스의 `readahead()` 시스템 콜은 파일의 내용을 프리패치하고 연속되는 파일 엑세스는 메인메모리에 있을 것이다.

### 10.9.2 Page Size

기존 머신의 운영체제 디자이너는 페이지 사이즈에 대한 선택권을 가진다. 그러나, 새로운 머신을 디자인하려면, 최상의 페이지 사이즈가 필요하다. 너가 예측했듯이, 단일 페이지 사이즈는 아닐 것이다. 페이지 크기는 2의 지수이고 2^12부터 2^22바이트까지 다양하다.

어떻게 페이지 크기를 선택할까? 한가지 고려사항은 페이지 테이블의 크기이다. 주어진 가상 메모리 공간에서, 페이지 테이블 사이즈를 감소시키는 것은 페이지의 수를 증가시키고 페이지 테이블의 크기도 증가한다. 4MB의 가상 메모리에서, 2^10 바이트인 2^12개의 페이지가 있고 2^13바이트인 2^9개의 페이지가 존재한다. 각 실행중인 프로세스가 페이지 테이블의 카피를 가져야하기 때문에, 큰 페이지 사이즈가 이상적이다.

메모리는 작은 페이지에서 조금더 효율적이다. 만약 프로세스가 00000에서 시작하고 필요한 만큼 계속된다면, 그것은 정확한 페이지 바운더리에서 끝나지 않는다. 그러므로, 마지막 페이지의 일부분은 할당이되지만, 사용되지는 않는다.(내부 단편화) 프로세스 크기와 페이지 크기의 독립성을 가정하면, 우리는 각 프로세스의 마지막 페이지는 절반을 버린다. 내부 단편화를 줄이기 위해서, 우리는 작은 페이지 크기가 필요하다.

다른 문제는 페이지를 읽고 쓰는데 걸리는 시간이다. 너가 11.1절에서 보게되듯이, 저장 장치인 HDD의 I/O 시간은 탐색, 지연, 전송시간으로 이루어진다. 전송시간은 전송되는 양에 비례하고, 작은 페이지 사이즈에 찬성하는 것이다. 그러나, 지연과 탐색 시간은 전송시간을 작게만든다. 전송 율이 초당 50MB이면, 512 바이트를 전달하는데 0.01미리초가 걸린다. 지연 시간은 아마 3미리초이고 탐색시간은 5미리 초이다. 여기서 전체 I/O시간은 8.01인데 0.1퍼센트만이 전송시간이다. 페이지 크기를 두배로 늘리면 8.02미리초이고 두 페이지를 읽는다면 16.02초이다. 이를 통해서 우리는 페이지 사이즈가 클수록 좋다는 것을 알 수 있다.

작은 페이지 사이즈는 지역성을 높인다. 작은 페이지 사이즈는 프로그램 지역성을 더욱 정확하게 매칭시킨다. 예를 들어서 200KB의 중에서 100KB를 실제로 사용하는 프로세스를 고려하겠다. 작은 페이지 사이즈는, 우리에게 더 좋은 **resolution**을 가지게하고, 실제로 필요한 메모리를 고립시켜준다. 큰 페이지 사이즈에서, 우리는 필요한 것 이외에도 전송을 하고 할당해야한다. 그러므로 작은 페이지 사이즈는 적은 I/O와 적은 총 할당 메모리를 결과한다.

페이지 크기가 1바이트이면, 우리는 매 바이트마다 페이지 폴트를 해야할 것이다. 200KB의 절반을 사용하는 프로세스는 102400의 페이지 폴트를 일으키는 것이다. 페이지 폴트는 인터럽트, 레지스터저장, 페이지 교체, 페이징 기기 큐잉, 테이블 업데이트의 오버헤드를 일으킨다. 페이지 폴트의 수를 줄이려면, 우리는 큰 페이지 사이즈를 가져야한다.

다른 요소가 고려되어야하기도 한다. 이 문제는 정답이 없다. 우리가 보았듯이 몇몇 요소는 작은 페이지(내부 단편화, 로컬리티)를 원하고, 큰 페이지(테이블 크기, I/O시간)을 원하기도 한다. 그럼에도 불구하고, 역사적인 트렌드는 큰 페이지 사이즈를 향했고, 모바일 시스템도 마찬가지이다. 실제로, 운영체제(1983)의 초판에서 최대 4096바이트의 페이지를 가졌는데, 이 수치는 1990년에 흔한 숫자는 아니었다. 현대 시스템들은 큰 사이즈를 가지게 되었다.

### 10.9.3 TLB Reach

9장에서, 우리는 TLB의 히트 율을 소개했다. TLB의 히트율은 가상 주소의 번역이 페이지 테이블이 아닌 TLB에서 해결되는 것을 의미한다. 명백히, 히트율은 TLB의 전체 엔트리 수에 관련있고, 히트율을 높이는 방법은 엔트리의 수를 높이는 것이다. 이것은, 그러나, 저렴하지 않은데, TLB를 설비하는 관계형 메모리는 비싸고 전력소모가 크다.

히트율과 관련된 것은 similar metric: **TLB reach**이다. TLB reach는 TLB로부터 접근 가능한 메모리의 양이고 페이지 크기에 엔트리의 수가 곱해진다. 이상적으로, 프로세스의 워킹 셋은 TLB에 저장되어야한다. 만약 그렇지 않으면 프로세스는 페이지 테이블에서 메모리 참조를 해석하느라 엄청난 시간을 사용한다. 만약 우리가 TLB 엔트리의 수를 두배로하면, 우리는 TLB 리치가 2배가 된다. 그러나, 몇몇 메모리를 많이 사용하는 앱에서는, 이것은 여전히 부족하다.

TLB리치를 증카시키는 다른 방법은 페이지의 사이즈를 늘리거나 다양한 페이지 사이즈를 제공하는 것이다. 만약 우리가 페이지 크기를 증가시키면, 4KB~16KB로 증가되면 4배의 TLB리치를 가진다. 그러나, 이것은 몇몇 앱에서 단편화를 증가시키게된다. 대안으로는, 대부분의 아키텍처는 하나 이상의 페이지 사이즈를 제공하므로, 운영체제는 이 것의 지원으로 장점을 얻는다.

9.7절에서 말했던 ARMv8 아키텍처는 페이지와 리전에 따라서 다른 사이즈를 제공한다. 추가적으로, 각 TLB 엔트리는 **contiguous bit**를 포함한다. 만약 비트가 특정 TLB 엔트리에 세팅되면, 엔트리는 연속된 메모리의 블록에 매핑된다. 연속 블럭의 가능한 나열은 단일 TLB에 매핑되고 TLB 리치를 증가시킨다.

1. 64KB TLB 엔트리는 16*4KB의 근접 블럭을 가진다.
2. 1GB TLB 엔트리는 32*32MB 근접 블럭을 가진다.
3. 2MB TLB 엔트리는 32*64KB 근접 블럭 또는 128\*16KB 근접 블럭을 가진다.

다중 페이지 크기 지원을 제공받으려면 하드웨어가 아닌 운영체제이 필요하다. 예를 들어서 TLB엔트리의 한 영역이 엔트리에 따라서 페이지 프레임의 크기를 가르키거나, ARM 아키텍처의 경우에는, 엔트리는 반드시 메모리의 연속된 블럭을 가르켜야한다. 하드웨어가 아닌 소프트웨어에서 TLB를 운영하는 것은 엄청난 비용이든다. 그러나 히트율의 증가와 TLB 리치 오프셋증가도 도움이 된다.

### 10.9.4 Inverted Pages Tables

9.4.3절에서 인버트 페이지 페이블을 소개했다. 페이지 관리의 목적은 가상-물리 주소 번역에 필요한 물리 메모리의 양을 줄이는 것이다. 우리는 이것은 각 물리 페이지에 맞는 엔트리를 가진 테이블을 만드는 것이고, <process-id, page-number>의 페어로 인덱싱하는 것이다.

우리가 가상 메모리 페이지를 각 물리 프레임에 맞게 유지하기 때문에, 인버트 페이지 테이블은 이 정보를 저장하는 물리 메모리의 양을 감소시킬 수 있다. 그러나, 인버트 페이지 테이블은 프로세스의 논리 주소 공간에 대한 완벽한 정보를 포함하지 않고, 정보는 만약 레퍼런스 페이지가 메모리에 없으면 정보가 필요하다. 디맨드 페이징은 프로세스가 페이지 폴트하려면 이 정보를 필요로한다. 정보가 가용해지면, 외부 페이지 테이블이 반드시 필요하다. 각 페이지 테이블은 전통적이 프로세스별 페이지 테이블이고 어디에 각 가상 페이지가 있는지 포함한다.

그러나 외부 페이지가 인버트 페이지의 성능을 감소시킬까? 이런 페이지들이 오직 페이지 폴트시에만 일어나면, 그들은 재빨리 존재할 필요가 없다. 대신에, 그들은 메모리가 필요할때 알아서 페이지 인/아웃된다. 불행히도, 페이지 폴트는 이제 가상 메모리 매니저가 다른 페이지 폴트를 생성하게한다.ㅇ 이 특별한 경우는 커널에서의 핸들링에 주의를 주고 페이지 룩업 프로세싱을 지연시킨다.

### 10.9.5 프로그램 구조

디맨드 페이징은 유저 프로그램에 투명하도록 디자인된다. 많은 사례에서, 유저는 메모리의 페이지된 환경에 확실히 무신경하다. 다른 경우에는, 시스템 성능은 만약 유저가 내포하는 디맨드 페이징을 인지하면 향상될 수 있다.

다음 예시를 살펴보겠다. 페이지가 128워드 사이즈라고 하겠다. C 프로그램에서 128-128 행렬을 0으로 초기화하는 함수를 보겠다. 이 코드는 다음과 같이 구성하면 어떨까?
```c
int i, j;
int[128][128] data;
for(int j=0; j<128;j++)
   for(int i=0; i<128; i++)
      data[i][j] = 0;
```
이 경우에는 행렬이 만약에 data[i]마다 페이지를 구성하면 128*128의 페이지 폴트가 일어날 것을 우리는 알고 있다. 그러므로 i와 j의 위치를 바꾸어 실행해서 128번의 페이지 폴트로 줄일 수 있다.

데이터 구조와 프로그래밍 구조의 세심한 선택은 지역성을 높이고 적은 페이지 폴트 율을 만들어낼 수 있다. 예를 들어서 스택은 좋은 지역성을 가지는데, 각 엑세스가 항상 탑에서만 일어나기 때문이다. 반면에, 해시 테이블은 분산된 참조를 가지고, 낮은 지역성을 생성한다. 물론, 레퍼러느의 지역성은 데이터 구조의 효율성의 한가지 측정이긴하다. 검색속도, 메모리 참조의 총 수, 페이지가 만져진 횟수 등이 있을 수도있다.

마지막 단계에서는, 컴파일러와 로드는 페이징에 큰 영향을 가진다. 코드와 데이터를 분리하고 reentrant 코드를 생성하는 것은 코드 페이지를 읽기 전용으로 만들고 결코 수정이 불가하다. 빈 페이지는 교체되기 위해서 반드시 페이지 아웃되지 않는다. 로더는 페이지 바운더리에 루틴 두는 것을 회피하고, 각 루틴을 완벽히 한 페이지에 둔다. 서로를 부르는 루틴은 같은 페이지에 패킹될수있다. 이 패키징은 빈-패킹 문제의 변형이다. 다양한 사이즈의 로드 세그먼트를 고정된 사이즈의 페이지에 패킹해서 인터페이지 참조를 최소화한다. 이런 접근은 특히 큰 페이지에서 유용하다.

### 10.9.6 I/O 인터락과 페이지 락킹

디맨드 페이징을 사용하면, 우리는 몇가지 페이지가 메모리에 **locked**되기를 원한다. 한가지 이런 상황은 I/O가 유저 메모리로부터 끝날 때 일어난다. I/O는 분리된 I/O 프로세서로 구현된다. 예를 들어서, USB를 위한 컨트롤러는 전송할 수 있는 바이트가 주어지고 버퍼의 메모리 주소를 제공한다. 전송이 끝나면, CPU가 인터럽트된다.

우리는 반드시 따라오는 순서가 일어나지 않는다고 확신해야한다. 프로세스는 I/O 리퀘스트를 이슈하고 I/O 기기 큐에 넣어진다. 그 동안에, CPU는 다른 프로세스에 주어진다. 이런 프로세스들은 페이지 폴트를 일으키고, 그들중의 하나는, 전역 교체 알고리즘을 사용하면, 대기 프로세스의 메모리 버퍼를 포함하는 페이지를 교체한다. 페이지들은 페이지 아웃된다. 조금 후에, I/O 요청이 기기 큐의 헤드로 도착하면, I/O는 특정 주소에서 일어난다. 그러나, 프레임은 다른 프로세스에 포함된 다른 페이지를 쓰고 있다.

이 문제에는 두가지 해결책이 있다. 하나는 유저 메모리에 I/O를 사용하지 않는 것이다. 대신에, 데이터들은 시스템 메모리와 유저 메모리 사이에서 복사된다. I/O는 오직 시스템 메모리와 I/O 기기에서만 자리하는 것이다. 그러므로, 테이프에 블럭을 쓰려면, 우리는 시스템 메모리에 블럭을 복사한 후에 테이프에 복사하는 것이다. 추가적인 복사는 용납할 수 없는 높은 오버헤드를 발생시킨다.

다른 해결책은 페이지가 메모리에 락되게 허용하는 것이다. 여기서, 락 비트는 모든 프레임에 있다. 만약 프레임이 락되면, 그것은 교체를 위해서 선택될 수 없다. 이 접근에서, 블럭에서 디스크로 쓸때, 우리는 블럭을 포함하는 메모리 페이지를 락한다. 시스템은 평소와 같게 진행된다. 락된 페이지들은 교체되지 않는다. I/O가 완료되면, 페이지들은 해제된다.

락 비트들은 다양한 상황에서 사용된다. 자주, 몇몇 또는 모든 운영체제 커널은 메모리에 잠금된다. 많은 운영체제들이 커널또는 특정 커널 모듈에 의해서 생긴 페이지 폴트를 허용하지 않는다. 유저 프로세스 또한 메모리에 락 페이지를 필요로 할 수 있다. 데이터베이스 프로세스는 메모리의 덩어리를 관리해야하고, 예를 들어서, 2차 저장소와 메모리에서 움직이는 블럭은 그것이 어떻게 데이터를 사용할지 최고의 지식을 가지고 있다. 메모리의 페이지 **pinning**은 꽤나 일반적인데, 대부분의 운영체제는 앱으로의 그것의 논리 주소공간이 핀될수있는 요청을 허용하는 시스템 콜을 가진다. 이 기능은 남용될수 있고 메모리 관리 알고리즘에 큰 스트레스를 유발할 수 있다. 그러므로, 앱은 이 요청을 만들기 위해서 특별한 특권을 필요로 한다.

락 비트의 다른 사용은 일반적인 페이지 교체를 포함한다. 다음 이벤트의 순서를 고려하겠다. 낮은 우선순위 프로세스가 폴트했다. 교체 프레임을 선택할 때, 페이징 시스템은 메모리에서 필요한 페이지를 읽는다. 계속할 준비가 되었고, 낮은 우선 순위 프로세스는 레디큐에 들어가고 CPU를 기다린다. 그것이 낮은 우선순위 프로세스이기에, 그것은 CPU 스케쥴러에게 한참 선택되지 않을 수 있다. 낮은 우선 순위 프로세스가 기다리는 동안, 높은 우선 순위 프로세스가 폴트한다. 교체할 것을 찾으면서, 페이징 시스템은 메모리에 있지만 참조되지 않고 수정되지 않은 페이지를 본다. 그것은 낮은 우선 순위 프로세스가 막 가지고온 페이지이다. 이 페이지는 완벽한 교체로 보인다. 그것은 깔끔하고 다시 쓰이지 않고, 오랜 시간동안 쓰지 않은 것 같다.

높은 순위 프로세스가 낮은 우선 순위 프로세스를 교체 가능한 것은 정책의 결정이다. 그 후에, 우리는 낮은 순위 프로세스를 늦추고 높은 순위 프로세스의 이득을 챙겨준다. 그러나, 낮은 우선 순위 프로세스의 페이지를 가져오는 노력은 낭비이다ㅏ. 만약 우리가 새롭게 가져온 페이지를 한번이라도 쓰도록 교체 알고리즘을 결정해야한다. 그러면 이 메커니즘에는 락 비트가 적용된다. 페이지가 교체를 위해서 선택되면, 락 비트가 켜진다. 그것은 폴팅 프로세스가 다시한번 실행될때까지 남아있다.

락 비트를 사용하는 것은 위험하다. 락비트는 꺼지지 않을 수 있다. 이 상황이 일어나면, 락된 프레임은 쓸수 없어진다. 예를 들어서, 솔라리스는 "hints"라는 락을 허용하는데, 그러나 그것은 만약 여유 프레임 풀이 작아지거나 개인 프로세스 리퀘스트가 많은 페이지 락을 요청하면 락을 무시해버린다.

## 10.10 운영체제 예제

### 10.10.1 리눅스

10.8.2 절에서, 우리는 리눅스가 어떻게 커널 메모리를 슬랩 할당으 이용해서 관리하는지 보았다. 우리는 이제 어떻게 리눅스가 가상메모리를 관리하는지 보겠다. 리눅스는 디맨드 페이징을 이용하고, 여유 프레임의 리스트로부터 페이지를 할당한다. 추가적으로, 그것은 LRU 근사화 클락 알고리즘을 이용한 전역 페이지 교체를 사용한다. 메모리를 관리하기 위해서, 리눅스는 두가지 타입의 페이지 리스트를 유지한다. `active_list`와 `inactive_list`이다. 액티브 리스트는 사용중이라고 고려되는 페이지를 포함하고, 인액티브 리스트는 최근에 참조되지 않고 재 사용가능한 것을 포함한다. 

각 페이지는 페이지가 참조될때마다 세팅되는 *accessed* 비트를 가진다.(실제 비트는 아키텍처에 의해서 달라질 수 있다.) 페이지가 처음 할당되면, 그것의 accessed 비트는 세팅되고, `active_list`의 rear에 추가된다. 비슷하게, `active_list`의 페이지가 참조될때마다, 페이지의 accessed 비트는 리셋된다. 시간을 지나서, lru 페이지는 `active_list`의 앞에 있게된다. 그로부터, 그것은 `incative_list`의 rear에 이주된다. 만약 `inactive_list`의 페이지가 참조되면, 그것은 `active_list`의 rear로 이동된다.

두 리스트들은 상대적인 밸런스를 유지하고 `active_list`는 `inactive_list`만큼 커지고, `active_list`의 front에 있는 페이지들은 `inactive_list`로 이동된다. 리눅스 커널은 주기적으로 시스템의 여유메모리를 깨우고 체크하는 페이지 아웃 다이몬 프로세스 `kswapd`를 가진다. 만약 여유 메모리가 특정 스레시홀드 아래로 떨어지면, `kswapd`는 `inactive_list`의 페이지를 스캔하고 여유 리스트로 재선언한다. 리눅스 가상 메모리 관리는 20장에서 더욱 자세히 다루겠다.

### 10.10.2 윈도우즈

윈도우 10은 32비트 64빝 시스템을 지원한다. 32비트 시스템에서, 프로세스의 가장 메모리 공간 주소는 2GB이고, 3GB까지 확장 가능하다. 32비트 시스템은 4GB의 물리 메모리를 지원한다. 64비트 시스템에서는, 윈도우 10은 128-TB 가상 주소 공간과 물리 메모리의 24TB를 지원한다. 윈도우 10은 대부분의 메모리 관리 기능인 공유 라이브러러, 디맨드 페이징, COW, 페이징, 메모리 압축 기술을 구현했다.

윈도우 10은 **clustering**이라는 디맨드 페이징을 구현했다. 이 전략은 메모리 참조의 로컬리티를 인지하고 폴팅 페이지뿐만이 아니라 따라오거나 앞서는 폴팅 페이지를 가지고 온다. 클러스터의 크기는 페이지 종류에 따라서 다양하다. 데이터 페이지에서, 클러스터는 3개의 페이지를 포함한다.(폴팅 페이지의 전, 후 페이지) 다른 페이지 폴트는 7개의 클러스터 크기를 가진다.

윈도우 가상 메모리 관리의 키 요소는 워킹 셋 관리이다. 프로세스가 생성되면, 그것은 최소 50페이지의 워킹셋과 최대 345개의 워킹셋에 할당된다. 이 **Working-set minimum**은 프로세스가 메모리를 가지도록 보장하는 페이지의 최소 수이다. 만약 충분한 메모리가 있으면, 프로세스는 그것의 **working-set maximum**을 할당한다. 프로세스가 **hard working-set limit**로 설정되면, 이런 값들은 무시된다. 프로세스는 충분한 메모리가 있으면 working-set maximum보다 커질 수 있다. 비슷하게, 프로세스에 할당된 메모리의 양은 메모리의 수요가 많을떄는 최소로 줄어들 수 있다.

윈도우즈는 LRU 근사화 클락 알고리즘을 사용하고, 지역/전역 페이지 교체 정책의 혼합을 사용한다. 가상 메모리 매니저는 여유 페이지 프레임의 리스트를 유지한다. 리스트는 스레시홀드 값은 충분한 여유메모리가 있는지 가르킨다. 만약 페이지 폴트가 그것의 워킹셋 맥시멈 이하인 프로세스에서 일어나면, 메모리 매니저는 여유 페이지의 리스트를 할당한다. 만약 워킹 셋 맥시멈인 프로세스가 페이지 폴트를 일으키고 충분한 메모리가 있으면, 프로세스는 페이지 폴트를 일으켜서 그것의 워킹셋맥시멈 이상으로 자라게한다. 만약 여유 메모리의 양이 부족하면, 커널은 반드시 프로세스의 워킹셋에서 지역 LRU교체를 통해서 교체해야한다.

여유 메모리의 양이 스레시홀드 아래로 떨어지면, 가상 메모리 매니저는 스레시홀드 위로 끌어올리기 위해서 **automatic working set trimming**이라고 알려진 전역 교체 전략을 사용한다. 만약 프로세스가 그것의 워킹셋 미니멈보다 많이 할당되면, 가상 메모리 매니저는 충분한 메모리가 가용할때까지 또는 프로세스가 워킹셋 미니멈에 도달할떄까지 워킹셋에서 페이지를 삭제한다. 작고 실행중인 프로세스 이전에 크고 유휴상태인 프로세스가 타겟이 된다. 손질하는 과정은 충분한 여유 메모리가 나올떄까지 지속되고, 필요하다면 이미 워킹셋 미니멈 아래인 프로세스로부터도 삭제한다. 윈도우는 워킹셋 트리밍을 유저 모드와 시스템 프로세스 둘다에 적용한다.

### 10.10.3 솔라리스

솔라리스에서, 스레드가 페이지 폴트를 일으키면, 커널은 여유 페이지의 리스트로부터 폴팅 스레드에 페이지를 할당한다. 그러므로, 커널이 여유 메모리를 유지하는 것은 반드시 해야하는 것이다. 여유 페이지의 리스트와 관련된 파라미터 `lotsfree`는 페이징을 시작하는 스레시홀드를 대표한다. `lotsfree` 파라미터는 물리 메모리의 1/64의 사이즈를 가진다. 초당 4번씩, 커널은 여유 메모리가 `lotsfree`이하인지 확인한다. 만약 프리 페이지의 수가 `lotsfree`아래로 떨어지면, **pageout**이라는 프로세스가 실행된다. 페이지아웃 프로세스는 세컨드 찬스 알고리즘과 비슷하고, 페이지를 두가지 hands로 스캐닝한다는 차이가 있다.

페이지아웃 프로세스는 다음과 같이 작동한다. 클락의 프론트 핸드가 메모리의 모든 페이지를 체크하고, 참조 비트를 0으로 만든다. 후에, 클락의 백 핸드가 메모리에 있는 페이지의 레퍼런스 비트를 확인하고, 여전히 참조비트가 0인 것을 프리 리스트에 보내고 만약 수정이 있다면 그것을 2차 저장소에 보관한다. 솔라리스는 프로세스가 만약 페이지가 다른 프로세스에 재할당 되기 전에 접근했으면 여유 리스트로부터 프로세스를 재선언하는 것을 허용함으로서 마이너 페이지 폴트를 관리한다.

페이지아웃 알고리즘은 페이지가 스캔되는 속도을 조절하기 위해서 다양한 파라미터를 사용한다. 스캔속도는 `slowscan`부터 `fastscan`으로 표현된다. 여유 메모리가 `lotsfree`이하로 떨어지면, 메모리의 여유에 따라서 `fastscan`또는 `slowscan`을 사용한다. `slowscan`의 기본 값은 초당 100페이지이다. `Fastscan`은 일반적으로 초당 전체 페이지의 절반으로 설정된다.(최대는 8192 페이지)

클락의 핸드 사이의 거리는 시스템 파라미터 `handspread`로 정해진다. 프론트 핸드의 클리어링과 백핸드의 값조사 사이의 시간은 `scanrate`와 `handspread`에 달려있다. 만약 `scanrate`가 초당 100페이지이고 `handspread`가 1024페이지라면, 프론트에 의해서 세팅된 것과 백핸드에 의해서 확인하는 시간은 10초의 간격을 가진다. 그러나, 메모리 시스템에서 놓인 수요때문에, 메모리 시스템에서 놓인 수요 때문에, `scanrate`는 일반적이지 않다. 이것은 클리어링과 탐색사이에 조금의 틈만 있다는 것이다.

말했듯이, 페이지 아웃 프로세스는 초당 4번 메모리를 체크한다. 그러나, 만약 메모리가 `desfree`(원하는 시스템의 여유메모리) 이하로 떨어지면, 페이지 아웃은 초당 수백번을 실행해서 적어도 `desfree`는 유지되도록 한다. 만약 페이지 아웃 프로세스가 30초간 `desfree`를 만족할수 없으면, 커널은 프로세스를 스와핑하기 시작하고, 스왑된 프로세스에 할당된 페이지를 모두 풀어버린다. 일반적으로, 커널은 긴시간동안 유휴인 프로세스를 찾는다. 만약 시스템이 `minfree`라는 최소값을 못지키면, 페이지 아웃 프로세스는 새로운 페이지에 대한 요청마다 실행된다.

페이지 스캐닝 알고리즘은 여러가지 프로세스에 의해서 공유되는 라이브러리 페이지를 스킵한다. 알고리즘은 또한 프로세스가 할당된 페이지와 정규 데이터 파일이 할당된 페이지를 구분한다. 이것은 **priority paging**이라하고 14.6.2절에서 언급하겠다.